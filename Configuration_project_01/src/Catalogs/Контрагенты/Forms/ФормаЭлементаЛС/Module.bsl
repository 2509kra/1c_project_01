...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		Объект.Партнер		= Параметры.ПараметрПартнер;
		Объект.НомерЛС		= Параметры.ПараметрНомерЛС;
		Объект.Наименование	= Параметры.ПараметрНаименование;
		Объект.ЮрФизЛицо	= Параметры.ПараметрЮрФизЛицо;
		Объект.ИНН			= Параметры.ПараметрИНН;
//		Объект.Наименование = Параметры.Наименование;
	...
	Если Объект.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ФизЛицо Тогда
		...
		Элементы.ИдДоговораЮЛ.Видимость = Ложь;
	Иначе
		//Элементы.ИдДоговораЮЛ.Видимость = Истина;
	...
	Если Параметры.Ключ.Пустая() И Не ПустаяСтрока(Параметры.ТекстЗаполнения) Тогда
	...
	//МИС (
	//Если в контактной информации заполнен емайл, то выведем его на первой странице	
	Емайл = ЭтаФорма["КонтактнаяИнформацияПоле" + СтрЗаменить(Справочники.ВидыКонтактнойИнформации.EmailКонтрагента.УникальныйИдентификатор(), "-", "x")+ 1];
	// ) МИС
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если Объект.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ФизЛицо Тогда
		...
		Иначе
			ТекЕмайл = "";
		КонецЕсли;	
		Если ТекЕмайл <> Емайл Тогда
			ПараметрыЗаписи.Вставить("Емайл",Емайл);		
		КонецЕсли;	
	КонецЕсли;	
	//) МИС
...
Процедура ПриОткрытии(Отказ)
	...
	ЭтотОбъект.ДатаЗадолженности = ДобавитьМесяц(ТекущаяДата(), -1);
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	//МИС (
	ИзмЕмайл = "";
	Если ПараметрыЗаписи.Свойство("Емайл",ИзмЕмайл) = Истина Тогда
		АЭС_Стек.ПередатьЕмайлВСтек(Объект.НомерЛС,ИзмЕмайл);	
	КонецЕсли;
	// )МИС
КонецПроцедуры
...
&НаСервере
Процедура ОбновитьДанныеПоЛСНаСервере(ЭлементДляОбновления = Неопределено)
	
	ЮЛФЛ = Неопределено;
	//Если ЗначениеЗаполнено(Объект.ЮридическоеФизическоеЛицо) Тогда
	//	ЮЛФЛ = Объект.ЮридическоеФизическоеЛицо;
	//Иначе
	//	Если ЗначениеЗаполнено(Объект.ЮрФизЛицо) Тогда
	//		Если Объект.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ФизЛицо 
	//			Или Объект.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ИндивидуальныйПредприниматель Тогда
	//			ЮЛФЛ = Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо;
	//		Иначе
	//			ЮЛФЛ = Перечисления.ЮридическоеФизическоеЛицо.ЮридическоеЛицо;
	//		КонецЕсли;
	//	КонецЕсли;
	//КонецЕсли;
	Если ЗначениеЗаполнено(Объект.ЮрФизЛицо) Тогда
		Если Объект.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ФизЛицо Тогда
			//ЮЛФЛ = Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо;
			СтруктураСтатуса  = АЭС_Стек.ПолучитьСтатусЛицевогоСчетаФЛ(Объект.НомерЛС, Неопределено);
			
			Если СтруктураСтатуса <> Неопределено Тогда
				
				Объект.АдресЛС				= СтруктураСтатуса.АдресЛС; 
				Объект.Наименование			= СтруктураСтатуса.ФИОЛС;
				Объект.НаименованиеПолное	= СтруктураСтатуса.ФИОЛС;
				
				//МИС (
				ПолученныйЕмайл                       = СтруктураСтатуса.ЕмайлЛС;
				Если НЕ ПустаяСтрока(ПолученныйЕмайл) Тогда
					Емайл = ПолученныйЕмайл;
					ЭтаФорма["КонтактнаяИнформацияПоле" + СтрЗаменить(Справочники.ВидыКонтактнойИнформации.EmailКонтрагента.УникальныйИдентификатор(), "-", "x")+ 1] = Емайл;
					ЭтаФорма.Модифицированность = Истина;
					ЭлементДляОбновления = "КонтактнаяИнформацияПоле" + СтрЗаменить(Справочники.ВидыКонтактнойИнформации.EmailКонтрагента.УникальныйИдентификатор(), "-", "x")+ 1;					
				Иначе					
					Сообщить("В СТЕК не задан e-mail для этого лицевого счета! Очистите поле вручную при необходимости!");
				КонецЕсли;	
				//) МИС
			КонецЕсли;	
		...
		Иначе
			//ЮЛФЛ = Перечисления.ЮридическоеФизическоеЛицо.ЮридическоеЛицо;
			ТаблицаСтатуса = АЭС_Стек.ПолучитьСтатусЛицевогоСчетаЮЛ(Объект.НомерЛС, Объект.ИНН, Объект.Партнер, Неопределено);
			...
			КонецЕсли;
			Если Ложь Тогда ТаблицаСтатуса = Новый ТаблицаЗначений; КонецЕсли;
			
			Если ТаблицаСтатуса <> Неопределено Тогда
				
				Объект.Наименование			= ТаблицаСтатуса[0].НаименованиеКраткое;
				Объект.НаименованиеПолное	= ТаблицаСтатуса[0].НаименованиеПолное;
				Объект.АдресЛС				= ТаблицаСтатуса[0].ФактическийАдрес; 
				Объект.ИНН					= ТаблицаСтатуса[0].ИНН; 
				Объект.КПП					= ТаблицаСтатуса[0].КПП; 
				//Объект.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ЮридическоеЛицо;
				//СуммаКассовыхПлатежей = 0;
				Если ТаблицаСтатуса.Количество() = 1 Тогда
					Объект.НомерЛС		= ТаблицаСтатуса[0].НомерДоговора;
					Объект.ИдДоговораЮЛ	= Формат(ТаблицаСтатуса[0].Ид, "ЧГ=0");
					//СуммаКассовыхПлатежей = ТаблицаСтатуса[0].СуммаКассовыхПлатежей;
				Иначе
					Элементы.НомерЛС.СписокВыбора.Очистить();
					
					Для Каждого стрТЗ Из ТаблицаСтатуса Цикл
						Если ЗначениеЗаполнено(стрТЗ.НомерДоговора) Тогда
							Элементы.НомерЛС.СписокВыбора.Добавить(стрТЗ.Ид, стрТЗ.НомерДоговора);
							//СуммаКассовыхПлатежей = СуммаКассовыхПлатежей + стрТЗ.СуммаКассовыхПлатежей;
						КонецЕсли;
					КонецЦикла;
					Если Элементы.НомерЛС.СписокВыбора.Количество()>0 Тогда 
						Элементы.НомерЛС.КнопкаСпискаВыбора = Истина;
						Элементы.НомерЛС.РежимВыбораИзСписка = Истина;
						
					КонецЕсли;
					Объект.ИдДоговораЮЛ = Неопределено;
				...
				//	ТекстСообщения = "Внимание! Сумма кассовых платежей превышает 100 000 р.! (" + Формат(СуммаКассовыхПлатежей, "ЧГ=3,0") + ") - " + Объект.НомерЛС + ", " + Объект.ИНН + ", " + Объект.Партнер;
				...
				//СуммаКассовыхПлатежей = Формат(СуммаКассовыхПлатежей, "ЧГ=3,0");
				//ЗначениеВРеквизитФормы(СуммаКассовыхПлатежей, "ТаблицаЗадолженностиЮЛ");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура ОбновитьДанныеПоЛС(Команда)
	
	Если ЗначениеЗаполнено(Объект.НомерЛС) Или ЗначениеЗаполнено(Объект.ИНН) Тогда
		ЭлементЭлПочта = Неопределено;
		ОбновитьДанныеПоЛСНаСервере(ЭлементЭлПочта);
		Если ЭлементЭлПочта <> Неопределено Тогда
			Подключаемый_КонтактнаяИнформацияПриИзменении(ЭтаФорма.Элементы.Найти(ЭлементЭлПочта));
		КонецЕсли;	
	Иначе
		Сообщить("Не заполнен номер лицевого счета");
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Процедура ОбновитьТаблицыЗадолженностиИОплатНаСервере()
	
	Если ЗначениеЗаполнено(Объект.ЮрФизЛицо) Тогда
		Если Объект.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ФизЛицо Тогда
			
			ТЗ1 = АЭС_Стек.ПолучитьТаблицуЗадолженностиФЛ(Объект.НомерЛС, Объект.ЮридическоеФизическоеЛицо, ЭтотОбъект.ДатаЗадолженности);
			
			Если ТипЗнч(ТЗ1) = Тип("ТаблицаЗначений") Тогда
				ЗначениеВРеквизитФормы(ТЗ1, "ТаблицаЗадолженностиФЛ"); 
			КонецЕсли;
			
			ТЗ2 = АЭС_Стек.ПолучитьТаблицуОплатФЛ(Объект.НомерЛС, Объект.ЮридическоеФизическоеЛицо, ЭтотОбъект.ДатаЗадолженности);
			
			Если ТипЗнч(ТЗ2) = Тип("ТаблицаЗначений") Тогда
				ЗначениеВРеквизитФормы(ТЗ2, "ТаблицаОплат"); 
			КонецЕсли;
			
		Иначе
	    	ТЗ = АЭС_Стек.ПолучитьТаблицуЗадолженностиЮЛ(Объект.ИдДоговораЮЛ, Объект.Партнер, ЭтотОбъект.ДатаЗадолженности);
			
			Если ТипЗнч(ТЗ) = Тип("ТаблицаЗначений") Тогда
				ЗначениеВРеквизитФормы(ТЗ, "ТаблицаЗадолженностиЮЛ"); 
			КонецЕсли;
...
&НаКлиенте
Процедура ОбновитьТаблицыЗадолженностиИОплат(Команда)
	ОбновитьТаблицыЗадолженностиИОплатНаСервере();
КонецПроцедуры

&НаКлиенте
Процедура ДатаЗадолженностиПриИзменении(Элемент)
	ОбновитьТаблицыЗадолженностиИОплатНаСервере();
КонецПроцедуры


&НаКлиенте
Процедура НомерЛСОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	
	НайденныйЭлемент = Элемент.СписокВыбора.НайтиПоЗначению(ВыбранноеЗначение);
	Если НайденныйЭлемент <> Неопределено Тогда
		СтандартнаяОбработка = Ложь;
		Объект.ИдДоговораЮЛ	= Формат(НайденныйЭлемент.Значение, "ЧГ=0");
		Объект.НомерЛС		= НайденныйЭлемент.Представление;
...
&НаСервере
Процедура ЕмайлПриИзмененииНаСервере(ЭлементЭлПочта)	
	ЭтаФорма["КонтактнаяИнформацияПоле" + СтрЗаменить(Справочники.ВидыКонтактнойИнформации.EmailКонтрагента.УникальныйИдентификатор(), "-", "x")+ 1] = Емайл;
	ЭлементЭлПочта = "КонтактнаяИнформацияПоле" + СтрЗаменить(Справочники.ВидыКонтактнойИнформации.EmailКонтрагента.УникальныйИдентификатор(), "-", "x")+ 1;
КонецПроцедуры
...
Процедура ЕмайлПриИзменении(Элемент)
	ЭлементЭлПочта = Неопределено;
	ЕмайлПриИзмененииНаСервере(ЭлементЭлПочта);	
	Если ЭлементЭлПочта <> Неопределено Тогда
		Подключаемый_КонтактнаяИнформацияПриИзменении(ЭтаФорма.Элементы.Найти(ЭлементЭлПочта));
	КонецЕсли;
	ЭтаФорма.Модифицированность = Истина;
