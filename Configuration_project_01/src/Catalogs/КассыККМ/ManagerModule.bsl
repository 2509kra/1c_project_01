...
Функция ПараметрыКассыККМ(КассаККМ) Экспорт
	...
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(НастройкиРМККассыККМ.ИспользоватьБезПодключенияОборудования, ЛОЖЬ)
	|			ТОГДА КассыККМ.СерийныйНомер
	|		ИНАЧЕ ВЫБОР
	|				КОГДА НастройкиРМККассыККМ.ПодключаемоеОборудование.СерийныйНомер <> """"
	|					ТОГДА НастройкиРМККассыККМ.ПодключаемоеОборудование.СерийныйНомер
	|				ИНАЧЕ КассыККМ.СерийныйНомер
	|			КОНЕЦ
	|	КОНЕЦ КАК СерийныйНомер,
	|	НастройкиРМККассыККМ.ПодключаемоеОборудование.ТипОборудования КАК ТипОборудования
	|ИЗ
	|	Справочник.КассыККМ КАК КассыККМ
	...
	//АМА
	ВозвращаемоеЗначение.Вставить("ТипОборудования",                        "");
	...
	Если Выборка.Следующий() Тогда
		...
		//АМА
		ВозвращаемоеЗначение.ТипОборудования                        = Выборка.ТипОборудования;
