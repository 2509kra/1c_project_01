...
Функция ПолучитьИтогиЗаПериодКнигаПродаж(СтруктураПараметров) Экспорт
	...
	|	ГДЕ
	|		НЕ НДСЗаписиКнигиПродажОбороты.ОтражатьВКниге = ЗНАЧЕНИЕ(Справочник.АЭС_ОтражениеНоменклатуры.НеОтражать)
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
...
//АЭС МИС (процедура стала экспортной, для внешней обработки Выгрузка счетов-фактур в XML
Функция ЗаписиКнигиПродаж(СтруктураПараметров) Экспорт
...
Функция ТекстЗапросаЗаписиРегистраНДСПродажи(СтруктураПараметров)
	...
	|   И НЕ НДСЗаписиКнигиПродаж.ОтражатьВКниге = ЗНАЧЕНИЕ(Справочник.АЭС_ОтражениеНоменклатуры.НеОтражать)
	|
	|СГРУППИРОВАТЬ ПО
