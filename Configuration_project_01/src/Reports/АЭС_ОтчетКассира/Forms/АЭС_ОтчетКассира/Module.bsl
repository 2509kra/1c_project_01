...
&НаСервере
Процедура КомандаСформироватьОтчетКассираНаСервере()
	Если ЭтотОбъект.Отчет.ДатаФормирования = Дата(1,1,1) Тогда 
		ЭтотОбъект.Отчет.ДатаФормирования = ТекущаяДата();
	КонецЕсли;
	ОтменаВыполнения = Ложь;
	Если ЭтотОбъект.Отчет.Касса.Пустая() Тогда 
		Сообщить("Незаполнен реквизит - КАССА");
		ОтменаВыполнения = Истина;
	КонецЕсли;
	Если ЭтотОбъект.Отчет.КассаККМ.Пустая() Тогда 
		Сообщить("Незаполнен реквизит - КассаККМ");
		ОтменаВыполнения = Истина;
	КонецЕсли;
	...
	КонецЕсли;
		...
		ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
		               ...
		               |	И НЕ ДенежныеСредства.ХозяйственнаяОперация В (ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереоценкаДенежныхСредств), ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПоступлениеДенежныхСредствИзДругойКассы), ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВыдачаДенежныхСредствВДругуюКассу), ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ОтражениеИзлишкаПриИнвентаризацииДенежныхСредств), ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ОтражениеНедостачиПриИнвентаризацииДенежныхСредств))
		               |
		               |СГРУППИРОВАТЬ ПО
		               |	ВЫБОР
		               |		КОГДА ДенежныеСредства.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
		               ...
		               |	И ЕСТЬNULL(ВЫРАЗИТЬ(ДенежныеСредства.Регистратор КАК Документ.ПриходныйКассовыйОрдер).КассаОтправитель.КассоваяКнига, ЗНАЧЕНИЕ(Справочник.КассовыеКниги.ПустаяСсылка)) <> &КассоваяКнига
		               |
		               |СГРУППИРОВАТЬ ПО
		               |	ВЫБОР
		               |		КОГДА ДенежныеСредства.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
		               ...
		               |	И ЕСТЬNULL(ВЫРАЗИТЬ(ДенежныеСредства.Регистратор КАК Документ.РасходныйКассовыйОрдер).КассаПолучатель.КассоваяКнига, ЗНАЧЕНИЕ(Справочник.КассовыеКниги.ПустаяСсылка)) <> &КассоваяКнига
		               |
		               |СГРУППИРОВАТЬ ПО
		               |	ВЫБОР
		               |		КОГДА ДенежныеСредства.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
		...
		Запрос.УстановитьПараметр("ДатаНачала", НачалоДня(ЭтотОбъект.Отчет.ДатаФормирования));
		Запрос.УстановитьПараметр("ДатаОкончания", КонецДня(ЭтотОбъект.Отчет.ДатаФормирования));
		Запрос.УстановитьПараметр("Организация", ЭтотОбъект.Отчет.КассаККМ.Владелец);
		Запрос.УстановитьПараметр("КассоваяКнига",ЭтотОбъект.Отчет.Касса.КассоваяКнига);
		...
		Иначе
			...
			стркт = ОтветственныеЛицаБППереопределяемый.ОтветственныеЛица(ЭтотОбъект.Отчет.КассаККМ.Владелец,ЭтотОбъект.Отчет.ДатаФормирования);	// Перечисления.ОтветственныеЛицаОрганизаций.ГлавныйБухгалтер
			ГлавныйБухгалтер = ФизическиеЛицаУТ.ФамилияИнициалыФизЛица(стркт.ГлавныйБухгалтер,ЭтотОбъект.Отчет.ДатаФормирования);
			//ГлавныйБухгалтер="";
			Кассир = "";
			Если ТипЗнч(ЭтотОбъект.Отчет.Кассир) = Тип("СправочникСсылка.ФизическиеЛица") Тогда 
				Кассир = ФизическиеЛицаУТ.ФамилияИнициалыФизЛица(ЭтотОбъект.Отчет.Кассир,ЭтотОбъект.Отчет.ДатаФормирования);
			ИначеЕсли ТипЗнч(ЭтотОбъект.Отчет.Кассир) = Тип("СправочникСсылка.Пользователи") Тогда 
				Кассир = ФизическиеЛицаУТ.ФамилияИнициалыФизЛица(ЭтотОбъект.Отчет.Кассир.Наименование);
			КонецЕсли;
			
			
			//"ФизическиеЛицаУТ.ФамилияИнициалыФизЛица(ВыборкаПоДокументамКассоваяКнига.Ответственный,ВыборкаПоДокументамКассоваяКнига.Дата)";
			
			
			Организация =ЭтотОбъект.Отчет.КассаККМ.Владелец;
		    КассоваяКнига = ЭтотОбъект.Отчет.Касса.КассоваяКнига;
			...
			ДатаНач = НачалоДня(ЭтотОбъект.Отчет.ДатаФормирования);
			ДатаКон = КонецДня(ЭтотОбъект.Отчет.ДатаФормирования);
			...
			ОбъОтчет = РеквизитФормыВЗначение("Отчет");
			Макет = ОбъОтчет.ПолучитьМакет("ПФ_MXL_ЛистКассовойКниги");
			...
			ШапкаОтчетаМини         = Макет.ПолучитьОбласть("ШапкаОтчетаМини|Отчет");
			
			ШапкаОтчетаМини.Параметры.ТекстШапки = "Отчет кассира по Участку " + ЭтотОбъект.Отчет.Касса.Подразделение + " за рабочую смену";
			ТабличныйДокумент.Вывести(ШапкаОтчетаМини);
...
#Область ОхуенскаяВстввка
		...
		//таНач = НачалоДня(ВыборкаПоДокументамКассоваяКнига.Дата); 
		...
		ЗапросПоДокументам.УстановитьПараметр("ТипЛиста", Перечисления.ТипыЛистовКассовойКниги.Обычный);
		ЗапросПоДокументам.УстановитьПараметр("МояТЗ", тз_з2);
		ЗапросПоДокументам.Выполнить();
		
		ЗапросПоДОкументам.Текст = "		
		|ВЫБРАТЬ
		|	вт_доки.документ КАК документ,
		|	НАЧАЛОПЕРИОДА(вт_доки.документ.Дата, ДЕНЬ) КАК День,
		|	вт_доки.документ.Дата КАК ДатаДок,
		|	вт_доки.документ.Номер КАК НомерДок,
		|	вт_доки.документ.Валюта КАК Валюта,
		|	ВЫБОР
		|		КОГДА вт_доки.документ ССЫЛКА Документ.ПриходныйКассовыйОрдер
		|			ТОГДА вт_доки.документ.ПринятоОт
		|		ИНАЧЕ вт_доки.документ.Выдать
		|	КОНЕЦ КАК ТекстДок,
		|	вт_доки.Приход КАК Приход,
		|	вт_доки.Расход КАК Расход,
		|	1 КАК НомерЛиста,
		|	0 КАК НомерСтроки,
		|	&ТипЛиста,
		|	ВЫРАЗИТЬ(вт_доки.документ.Основание КАК Строка(200)) КАК Основание,
		|	вт_доки.КорреспондирующийСчет КАК Счет
		|ИЗ
		|	вт_доки КАК вт_доки
		|ГДЕ
		|	вт_доки.документ.Проведен
		|	И ТИПЗНАЧЕНИЯ(вт_доки.документ) В (ТИП(Документ.ПриходныйКассовыйОрдер), ТИП(Документ.РасходныйКассовыйОрдер))
		|
		|УПОРЯДОЧИТЬ ПО
		|	День,
		|	НомерЛиста,
	...
	//		ЛистовЗаГод = 1 стр
			...
			Если ДатаЛиста >= ДатаНач Тогда
				...
				ОбластьПодвалОтчет.Параметры.ГлБухгалтер = ГлавныйБухгалтер;
				ОбластьПодвалОтчет.Параметры.Кассир =  Кассир;
...
&НаКлиенте
Процедура КомандаСформироватьОтчетКассира(Команда)
	КомандаСформироватьОтчетКассираНаСервере();
КонецПроцедуры
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	фл = ПараметрыСеанса.ТекущийПользователь.ФизическоеЛицо;
	Если не фл.Пустая() Тогда 
		ЭтотОбъект.Отчет.Кассир = фл;
	Иначе 
		ЭтотОбъект.Отчет.Кассир = тп;
	КонецЕсли;
	#КонецОбласти
	...
	//КонецЕсли;
	Если РольДоступна("АЭС_ПолныеПрава") Тогда
		//	Элементы.КассаККМ.КнопкаОчистки = Истина;
		Элементы.Команда1_тест_параметры.Видимость = Истина;
	Иначе 
		Элементы.Команда1_тест_параметры.Видимость = Ложь;
	КонецЕсли;
...
&НаСервере
Процедура Команда1_тест_параметрыНаСервере()
	ЭтотОбъект.Отчет.ДатаФормирования = Дата(2017,7,11);
	ЭтотОбъект.Отчет.Касса = Справочники.Кассы.НайтиПоНаименованию("Основная касса ВО Советский участок");
	ЭтотОбъект.Отчет.КассаККМ = Справочники.КассыККМ.НайтиПоНаименованию("кас. /аппарат №0439740008021277 через Репринцева А.С. кассира");
	
КонецПроцедуры

&НаКлиенте
Процедура Команда1_тест_параметры(Команда)
	Команда1_тест_параметрыНаСервере();
КонецПроцедуры
