...
Процедура ПолучитьБлокировкиНДС(ПараметрыРасчета) Экспорт
	...
	|ВЫБРАТЬ
	|	БлокировкаВычетаНДССчетаФактуры.СчетФактура КАК СчетФактура,
	|	БлокировкаВычетаНДССчетаФактуры.Ссылка КАК Ссылка
	...
	|		ВЫРАЗИТЬ(ОплаченныеЭлектронныеУслуги.НДСПриобретения * ((ОплаченныеЭлектронныеУслуги.СуммаПриобретения - ОплаченныеЭлектронныеУслуги.СуммаОплаты) / ОплаченныеЭлектронныеУслуги.СуммаПриобретения) КАК ЧИСЛО(15, 2))
	|	ИЗ
	...
	Для Каждого ПогашенныйАвансПоСФ Из СтрокиЗачетаАванса Цикл
		...
		Для Каждого СтрокаАванса Из СтрокиАвансаНачислениеНДС Цикл
			...
			Если СтрокаАванса.ВключатьВКнигуПродаж Тогда
				...
				Если СтрокаАванса.КодВидаОперации = "26" Тогда 
				//)
