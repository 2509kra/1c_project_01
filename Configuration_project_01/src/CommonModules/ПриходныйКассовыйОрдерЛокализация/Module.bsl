...
Функция СформироватьПечатнуюФормуКО1(МассивОбъектов, ОбъектыПечати, ПараметрыПечати)
	...
	|			ТОГДА ВЫБОР
	|					КОГДА ДанныеДокумента.АгентскийПлатеж
	|						ТОГДА ВЫБОР
	|								КОГДА КассыДополнительныеРеквизиты.Значение = ИСТИНА
	|									ТОГДА ""50.04.2""
	|								ИНАЧЕ ""50.04.1""
	|							КОНЕЦ
	|					ИНАЧЕ ВЫБОР
	...
	|	СтруктураПредприятияДополнительныеРеквизиты.Значение КАК КодПодразделения,
	|	ЕстьNull(КассыДополнительныеРеквизиты.Значение, ЛОЖЬ) КАК Операционная
	|ИЗ
	...
	|		ПО ДанныеДокумента.Касса = КассыДополнительныеРеквизиты.Ссылка.Ссылка
	|			И (КассыДополнительныеРеквизиты.Свойство.Заголовок = ""Операционная"")
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СтруктураПредприятия.ДополнительныеРеквизиты КАК СтруктураПредприятияДополнительныеРеквизиты
	|		ПО ДанныеДокумента.Касса.Подразделение = СтруктураПредприятияДополнительныеРеквизиты.Ссылка.Ссылка
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.Ссылка.АгентскийПлатеж И Выборка.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоступлениеДенежныхСредствИзДругойКассы Тогда
			ОбластьМакета.Параметры.СубСчет = "50.04.2";	//АМА
		ИначеЕсли Выборка.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоступлениеДенежныхСредствИзДругойКассы Тогда
			ОбластьМакета.Параметры.СубСчет = Справочники.СтатьиДвиженияДенежныхСредств.ПоступлениеДенежныхСредствИзДругойКассы.КорреспондирующийСчет;			
		Иначе
