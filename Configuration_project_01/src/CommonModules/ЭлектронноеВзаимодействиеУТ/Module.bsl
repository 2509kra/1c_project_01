...
Процедура ЗаполнитьДанныеПервичногоДокументаУПД(СтруктураДанных, СтруктураЭД, ДеревоДанных, Отказ) Экспорт
	...
	Пока СтрокаТовары.Следующий() Цикл
		...
		Если (ТипЗнч(ДокументОтгрузки) = Тип("ДокументСсылка.РеализацияТоваровУслуг") 
			...
			КонецЕсли;
		ИначеЕсли ТипЗнч(ДокументОтгрузки) = Тип("ДокументСсылка.КорректировкаРеализации") И ИскатьУпаковку Тогда
			Если ДокументОтгрузки.Товары.Количество() >= СтрокаТовары.НомерСтроки Тогда			
				Если СтрокаТовары.НомерСтроки > 0 Тогда
					СтрокаТабЧасти = ДокументОтгрузки.Товары[СтрокаТовары.НомерСтроки - 1];
				Иначе
					СтрокаТабЧасти = ДокументОтгрузки.Товары[СтрокаТовары.НомерСтроки];
...
Процедура ЗаполнитьДанныеПервичногоДокументаУКД(СтруктураДанных, СтруктураЭД, ДеревоДанных) Экспорт
	...
	Пока СтрокаТовары.Следующий() Цикл
		...
		Если ТипЗнч(ДокументКорректировки) = Тип("ДокументСсылка.КорректировкаРеализации") Тогда
			СтруктураПоискаСтроки = Новый Структура("Номенклатура, Характеристика");
			ЗаполнитьЗначенияСвойств(СтруктураПоискаСтроки, СтрокаТовары);
			НайденныеСтроки = ДокументКорректировки.Товары.НайтиСтроки(СтруктураПоискаСтроки);
			Если НайденныеСтроки.Количество() > 0 Тогда
...
Процедура ЗаполнитьДанныеПервичногоДокументаУПД_2019(СтруктураДанных, СтруктураЭД, ДеревоДанных, Отказ) Экспорт
	...
	//МИС у документа Запись книги продаж нет ревизита договор
	Если ЗначениеЗаполнено(ДокументОтгрузки) И ОбщегоНазначения.ЕстьРеквизитОбъекта("Договор",ДокументОтгрузки.Метаданные()) Тогда
	...
	//мис Не отрабатывает, если запись книги продаж
	Если ТипЗнч(ДокументОтгрузки) = Тип("ДокументСсылка.РеализацияУслугПрочихАктивов") или ТипЗнч(ДокументОтгрузки) = Тип("ДокументСсылка.ЗаписьКнигиПродаж") Тогда
	...
	Пока СтрокаТовары.Следующий() Цикл
		...
		Если (ТипЗнч(ДокументОтгрузки) = Тип("ДокументСсылка.РеализацияТоваровУслуг") 
				Или ТипЗнч(ДокументОтгрузки) = Тип("ДокументСсылка.ВозвратТоваровПоставщику")) И ИскатьУпаковку Тогда
			Если ЗначениеЗаполнено(ДокументОтгрузки.ВидыЗапасов) Тогда 	
				Если СтрокаТовары.НомерСтроки > 0 Тогда
					СтрокаТабЧасти = ДокументОтгрузки.ВидыЗапасов[СтрокаТовары.НомерСтроки - 1];
				Иначе
					СтрокаТабЧасти = ДокументОтгрузки.ВидыЗапасов[СтрокаТовары.НомерСтроки];
				КонецЕсли;
			Иначе
				Если СтрокаТовары.НомерСтроки > 0 Тогда
					СтрокаТабЧасти = ДокументОтгрузки.Товары[СтрокаТовары.НомерСтроки - 1];
		...
		ИначеЕсли ТипЗнч(ДокументОтгрузки) = Тип("ДокументСсылка.КорректировкаРеализации") И ИскатьУпаковку Тогда
			СтруктураПоискаСтроки = Новый Структура("Номенклатура, Характеристика");
			ЗаполнитьЗначенияСвойств(СтруктураПоискаСтроки, СтрокаТовары);
			НайденныеСтроки = ДокументОтгрузки.Товары.НайтиСтроки(СтруктураПоискаСтроки);
			Если НайденныеСтроки.Количество() > 0 Тогда
				СтрокаТабЧасти = НайденныеСтроки[0];
				УпаковкаИзДокумента = СтрокаТабЧасти.Упаковка;
			Иначе
	...
	Если ТипЗнч(ДокументОтгрузки) <> Тип("ДокументСсылка.КорректировкаРеализации")
			...
			И ТипЗнч(ДокументОтгрузки) <> Тип("ДокументСсылка.ВозвратТоваровМеждуОрганизациями") 
			И ТипЗнч(ДокументОтгрузки) <> Тип("ДокументСсылка.ПередачаТоваровМеждуОрганизациями") 
			И ТипЗнч(ДокументОтгрузки) <> Тип("ДокументСсылка.ЗаписьКнигиПродаж")  //МИС
			И Не ВидСчетаФактуры = "Авансовый" Тогда
		ДокументОснование = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОтгрузки, "ЗаказКлиента");
	КонецЕсли;
	Если ЗначениеЗаполнено(ДокументОснование) Тогда
