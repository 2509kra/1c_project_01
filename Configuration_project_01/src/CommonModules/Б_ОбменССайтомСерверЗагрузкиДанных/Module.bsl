...
Функция АТОМ_НайтиОрганизацияПоКПП(КППОрганизации)
	
	Возврат	Справочники.Организации.НайтиПоРеквизиту("КПП",КППОрганизации);
	
	
КонецФункции	

Функция АТОМ_ОпределитьСкладВЗаказ(идСкладаИзДокумента)
		
	Склад = Справочники.Склады.ПустаяСсылка();
	Разделитель	= СтрНайти(идСкладаИзДокумента,"#");
	идСклада = СокрЛП(Сред(идСкладаИзДокумента,4,80));
	
	ПунктВыдачи	= Справочники.АЭС_ПунктыВыдачи.ПолучитьСсылку(Новый УникальныйИдентификатор(идСклада));
	
	Если ЗначениеЗаполнено(ПунктВыдачи.Код) Тогда
		
		РозничныйСклад	= ПунктВыдачи.Склады.Найти(Истина,"Розничный");
		
		Если РозничныйСклад	= Неопределено Тогда
			Склад	= ПунктВыдачи.Склады[0].Склад;	
		Иначе
			Склад	= РозничныйСклад;		
		КонецЕсли; // Если РозничныйСклад	= Неопределено Тогда	
		
	КонецЕсли; // Если ЗначениеЗаполнено(ПунктВыдачи.Код) Тогда	
	
	Возврат Склад
	
КонецФункции // Функция АТОМ_ОпределитьСкладВЗаказ(идСкладаИзДокумента)

Функция АТОМ_ОпределитьПунктВыдачи(Склад)
	
	ПунктВыдачи	= Справочники.АЭС_ПунктыВыдачи.ПустаяСсылка();
	
	Запрос	= Новый Запрос;
	ТекстЗапроса	= "
		|ВЫБРАТЬ
		|	АЭС_ПунктыВыдачиСклады.Ссылка КАК Ссылка
		|ИЗ
		|	Справочник.АЭС_ПунктыВыдачи.Склады КАК АЭС_ПунктыВыдачиСклады
		|ГДЕ
		|	АЭС_ПунктыВыдачиСклады.Склад = &Склад";
	...
	Пока РЗ.Следующий() Цикл
		...
		ПунктВыдачи	= РЗ.Ссылка;	
		
	КонецЦикла; // Пока РЗ.Следующий() Цикл	
		
	Возврат ПунктВыдачи;
	
КонецФункции // ункция АТОМ_ОпределитьПунктВыдачи(идСкладаИзДокумента)

Функция АТОМ_ПолучитьСкладИзПунктовВыдачи(идПунктаВыдачи)
	
	
	//Склад = Справочники.Склады.ПустаяСсылка();
	//Разделитель	= СтрНайти(идПунктаВыдачи,"#");
	//идСклада = СокрЛП(Сред(идПунктаВыдачи,4,80));
	//
	//ПунктВыдачи	= Справочники.АЭС_ПунктыВыдачи.ПолучитьСсылку(Новый УникальныйИдентификатор(идСклада));
	...
	//Разделитель	= СтрНайти(идПредложения,"#");
	//
	//идПредложения = СокрЛП(Сред(идПредложения,Разделитель+1,80));
	//
	//Организация	= Справочники.Организации.ПолучитьСсылку(Новый УникальныйИдентификатор(идПредложения));
	//		
	//Возврат Организация
		
КонецФункции // Функция АТОМ_ПолучитьОрганизациюИзТорговогоПредложения(идПредложения)

Функция АТОМ_ПолучитьОрганизациюИзТорговогоПредложения(идПредложения)
	
	Запрос	= Новый Запрос;
	ТекстЗапроса	= "
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ ПЕРВЫЕ 1
		|	АЭС_ТорговыеПредложенияСрезПоследних.Организация КАК Организация
		|ИЗ
		|	РегистрСведений.АЭС_ТорговыеПредложения.СрезПоследних(, Контрагент = &Контрагент) КАК АЭС_ТорговыеПредложенияСрезПоследних";
	...
	Попытка
		Организация	= Справочники.Организации.ПолучитьСсылку(Новый УникальныйИдентификатор(идПредложения));
		Контрагент	= Справочники.Контрагенты.ПолучитьСсылку(Новый УникальныйИдентификатор(идПредложения));
		
		Если ЗначениеЗаполнено(Контрагент) Тогда
			Запрос.Текст	= ТекстЗапроса;
			Запрос.УстановитьПараметр("Контрагент",Контрагент);
			РЗ_Организация	= Запрос.Выполнить().Выбрать();
			Пока РЗ_Организация.Следующий() Цикл
				
				Организация	= РЗ_Организация.Организация;
				
			КонецЦикла;	// Пока РЗ_Организация.Следующий() Цикл
			
		КонецЕсли; // Если ЗначениеЗаполнено(Контрагент) Тогда	
...
Функция АТОМ_ПолучитьВидТорговогоПредложения(идПредложения)
	
	ВидТорговогоПредложения	= Перечисления.АЭС_ВидТорговогоПредложения.Атом; // По умолчанию
	...
	Попытка
		...
		Если НЕ ЗначениеЗаполнено(Организация.ИНН) Тогда
			ВидТорговогоПредложения	= Перечисления.АЭС_ВидТорговогоПредложения.Консигнация;
	...
	Возврат ВидТорговогоПредложения;	
	
КонецФункции // Функция АТОМ_ПолучитьВидТорговогоПредложения(идПредложения)
	
Функция АТОМ_ПолучитьПартнера(идПартнера)
	...
	ТекстЗапроса	= "
		...
		|ГДЕ
		|	Партнеры.НаименованиеПолное = &Наименование";
...
КонецФункции // Функция АТОМ_ПолучитьПартнера(идПартнера)

Функция АТОМ_ПолучитьСоглашениеПоОрганизации(Организация,ДатаСреза)
	...
	ТекстЗапроса	= "
		...
		|ИЗ
		|	РегистрСведений.АЭС_ДополнительныеНастройкиИМ.СрезПоследних(&ДатаСреза, Организация = &Организация) КАК Соглашения";
	
	Запрос.Текст	= ТекстЗапроса;
	Запрос.УстановитьПараметр("Организация",Организация);
	Запрос.УстановитьПараметр("ДатаСреза",ДатаСреза);
	
	рзЗапроса	= Запрос.Выполнить().Выбрать();
	
	Пока рзЗапроса.Следующий() Цикл
		
		Соглашение	= рзЗапроса.Соглашение;
		
	КонецЦикла; // Пока рзЗапроса.Следующий() Цикл	
				
	Возврат Соглашение;
		
КонецФункции // Функция АТОМ_ПолучитьОрганизациюИзТорговогоПредложения(идПредложения)

Процедура АТОМ_СоздатьЗадачуНаОбработкуЗаказа(ПараметрыЗадачи)
	
	Запрос	= Новый Запрос;
	ТекстЗапроса	= "
		|ВЫБРАТЬ
		|	АЭС_ПунктыВыдачиПользователи.Пользователь КАК Исполнитель
		|ПОМЕСТИТЬ вт_Рассылка	
		|ИЗ
		|	Справочник.АЭС_ПунктыВыдачи.Склады КАК АЭС_ПунктыВыдачиСклады
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.АЭС_ПунктыВыдачи.Пользователи КАК АЭС_ПунктыВыдачиПользователи
		|		ПО АЭС_ПунктыВыдачиСклады.Ссылка = АЭС_ПунктыВыдачиПользователи.Ссылка		
		|ГДЕ
		|	АЭС_ПунктыВыдачиСклады.Склад = &Склад
		|	И АЭС_ПунктыВыдачиСклады.Ссылка.ФормироватьЗаданияПользователям
		|;
		|///////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ЗадачаИсполнителяЗадачиПоИсполнителю.Исполнитель КАК Исполнитель,
		|	ЗадачаИсполнителяЗадачиПоИсполнителю.Предмет КАК Заказ,
		|	Ложь КАК СоздатьЗадачу
		|ПОМЕСТИТЬ вт_Задачи	
		|ИЗ
		|	Задача.ЗадачаИсполнителя.ЗадачиПоИсполнителю КАК ЗадачаИсполнителяЗадачиПоИсполнителю
		|ГДЕ
		|	ЗадачаИсполнителяЗадачиПоИсполнителю.Предмет = &Заказ	
		|;
		|//////////////////////////////////////
		|
		|ВЫБРАТЬ 
		|	Рассылка.Исполнитель,
		|	ЕСТЬNULL(Задачи.СоздатьЗадачу,Истина) КАК СоздатьЗадачу	
		|ИЗ 
		|	вт_Рассылка КАК Рассылка
		|	ЛЕВОЕ СОЕДИНЕНИЕ вт_Задачи КАК Задачи
		|		ПО (Рассылка.Исполнитель = Задачи.Исполнитель)
		|	";
	
	Запрос.Текст	= ТекстЗапроса;
	Запрос.УстановитьПараметр("Склад",ПараметрыЗадачи.Склад);
	Запрос.УстановитьПараметр("Заказ",ПараметрыЗадачи.Заказ);
	
	рзИсполнители	= Запрос.Выполнить().Выбрать();
	
	Пока рзИсполнители.Следующий() Цикл
		
		Если рзИсполнители.СоздатьЗадачу Тогда
			
			НоваяЗадача	= БизнесПроцессы.Задание.СоздатьБизнесПроцесс();
		
			НоваяЗадача.Исполнитель		= рзИсполнители.Исполнитель;
			//НоваяЗадача.ДатаНачала		= ТекущаяДата();
			НоваяЗадача.Дата			= ТекущаяДата();
			НоваяЗадача.Наименование	= "Поступил заказ с интернет магазина № " + ПараметрыЗадачи.Заказ.НомерПоДаннымКлиента ;
			НоваяЗадача.Предмет			= ПараметрыЗадачи.Заказ;
			//НоваяЗадача.ПредметСтрокой	= ПараметрыЗадачи.Заказ;
			//НоваяЗадача.Описание		= ПараметрыЗадачи.ОписаниеЗадачи;
			Попытка
	...
	КонецЦикла; // Пока рзИсполнители.Следующий() Цикл	

КонецПроцедуры // Процедура АТОМ_СоздатьЗадачуНаОбработкуЗаказа(ПараметрыЗадачи)	

Функция АТОМ_ПолучитьБанковскийСчетОрганизацииНаДату(ДатаСреза,Организация)
	...
	ТекстЗапроса	= "
		...
		|ИЗ
		|	РегистрСведений.АЭС_ДополнительныеНастройкиИМ.СрезПоследних(&ДатаСреза, Организация = &Организация) КАК БанковскиеСчета";
	...
	ВыборкаБанк	= Запрос.Выполнить().Выбрать();
	
	Пока ВыборкаБанк.Следующий() Цикл
		
		БанковскийСчет =ВыборкаБанк.БанковскийСчет;
		Прервать;
		
	КонецЦикла; // Пока ВыборкаБанк.Следующий() Цикл
	
	Возврат БанковскийСчет;
	
КонецФункции // Функция АТОМ_ПолучитьБанковскийСчетОрганизацииНаДату(ДатаСреза,Организация)	

Процедура АТОМ_СозданиеПеремещенийДляСозданногоЗаказа(ЗаказКлиента,ОписаниеЗадачи)
	
	НазначениеЗаказа = Справочники.Назначения.ПустаяСсылка();
	ТелоОписанияЗадачи	= "";
	...
	РЗ_Назначения	= Запрос.Выполнить().Выбрать();
	Пока РЗ_Назначения.Следующий() Цикл
		
		НазначениеЗаказа = РЗ_Назначения.Ссылка;
			
	КонецЦикла; // Пока РЗ_Назначения.Следующий() Цикл	
		
	ПунктВыдачи	= АТОМ_ОпределитьПунктВыдачи(ЗаказКлиента.Склад);
	
	ЗаказТовары = ЗаказКлиента.Товары.ВыгрузитьКолонку("Номенклатура");
	...
	ТекстЗапроса	= "
		...
		|					спр.Ссылка
		|				ИЗ
		|					Справочник.Организации КАК спр)) КАК Документ
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ПриоритетыСкладов.Склад КАК Склад,
		|	ПриоритетыСкладов.Приоритет КАК Приоритет,
		|	ПриоритетыСкладов.ПунктВыдачи КАК ПунктВыдачи
		|ПОМЕСТИТЬ Рег	
		|ИЗ
		...
		|					Рег.Регистратор
		|				ИЗ
		|					ДокРегистратор КАК Рег) И ПунктВыдачи = &ПунктВыдачи 
		|					) КАК ПриоритетыСкладов
		|
		|;
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	Рег.Склад КАК Склад,
		|	Рег.Склад.СкладОтветственногоХранения КАК СкладОтветственногоХранения,
		|	Рег.ПунктВыдачи КАК ПунктВыдачи,
		|	Рег.Приоритет КАК Приоритет,
		|	СвободныеОстаткиОстатки.ВНаличииОстаток - ЕСТЬNULL(СвободныеОстаткиОстатки.ВРезервеПодЗаказОстаток, 0) КАК ВНаличииОстаток
		|ИЗ
		|	Рег КАК Рег
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.СвободныеОстатки.Остатки(&ДатаСреза, Номенклатура В (&мНоменклатуры)) КАК СвободныеОстаткиОстатки
		|		ПО Рег.Склад = СвободныеОстаткиОстатки.Склад
		|ГДЕ
		|	СвободныеОстаткиОстатки.ВНаличииОстаток - ЕСТЬNULL(СвободныеОстаткиОстатки.ВРезервеПодЗаказОстаток, 0) <> 0
		|
		|УПОРЯДОЧИТЬ ПО
	...
	Запрос.УстановитьПараметр("ДатаСреза",		ЗаказКлиента.Дата);		
	Запрос.УстановитьПараметр("мНоменклатуры",	ЗаказТовары);
	Запрос.УстановитьПараметр("ПунктВыдачи",	ПунктВыдачи);
	
	РЗ_Остатки	= Запрос.Выполнить().Выгрузить();
		
	РаспределеннаяТаблица	= Новый ТаблицаЗначений;
	РаспределеннаяТаблица.Колонки.Добавить("Склад",			Новый ОписаниеТипов("СправочникСсылка.Склады"));
	РаспределеннаяТаблица.Колонки.Добавить("Количество",	Новый ОписаниеТипов("Число"));
	РаспределеннаяТаблица.Колонки.Добавить("Номенклатура",	Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
	...
	Для Каждого стр Из ЗаказКлиента.Товары Цикл
		...
		мНайденныхСтрок = РЗ_Остатки.НайтиСтроки(ПараметрыПоиска);
		
		Для Каждого стрОстатков	 Из мНайденныхСтрок Цикл
			
			Распределено 			= Мин(ОсталосьРаспределить, стрОстатков.ВНаличииОстаток);
			ОсталосьРаспределить	= ОсталосьРаспределить - Распределено;
			
			НоваяСтрока	= РаспределеннаяТаблица.Добавить();
			НоваяСтрока.Склад			= стрОстатков.Склад;
			НоваяСтрока.Количество		= Распределено;
			НоваяСтрока.Номенклатура	= стр.Номенклатура;
		
			Если ОсталосьРаспределить = 0 Тогда
				Прервать;
			КонецЕсли; // Если ОсталосьРаспределить = 0 Тогда
				
		КонецЦикла; // Для Каждого стрОстатков	 Из мНайденныхСтрок Цикл	
			
	КонецЦикла; // Для Каждого стр Из ЗаказКлиента.Товары Цикл	
	...
	ТекстЗапроса = "
		...
		|/////////////////////////////
		|ВЫБРАТЬ 
		|    РаспределеннаяТаблица.Склад,
		|    РаспределеннаяТаблица.Номенклатура,
		|    РаспределеннаяТаблица.Количество,
		|    РаспределеннаяТаблица.Количество КАК КоличествоУпаковок
		|ИЗ 
		|	ВременнаяТаблица КАК РаспределеннаяТаблица	
		|ИТОГИ ПО
		|	Склад";
	
	Запрос.Текст = ТекстЗапроса;
	Запрос.УстановитьПараметр("ТаблицаЗначений",РаспределеннаяТаблица);
	ВыборкаПоСкладам = Запрос.Выполнить().Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	
	Если РаспределеннаяТаблица.Количество() <> 0 Тогда
		
		НовыйДокНазначение = Документы.КорректировкаНазначенияТоваров.СоздатьДокумент();
		
		НовыйДокНазначение.Организация	= ЗаказКлиента.Организация;
		НовыйДокНазначение.Дата			= ЗаказКлиента.Дата;
		НовыйДокНазначение.ВидОперации	= Перечисления.ВидыОперацийКорректировкиНазначения.Резервировать;
		НовыйДокНазначение.Назначение	= НазначениеЗаказа;
		НовыйДокНазначение.Комментарий 	= "#Создан автоматически на основании заказа: "+ЗаказКлиента.Номер;
		...
		Пока ВыборкаПоСкладам.Следующий() Цикл
			...
			Пока ВыборкаПоТоварам.Следующий() Цикл
				...
				нСтр	= НовыйДокНазначение.Товары.Добавить();
				
					
				нСтр.Номенклатура			= ВыборкаПоТоварам.Номенклатура;
				нСтр.Заказ					= ЗаказКлиента;
				нСтр.Количество				= ВыборкаПоТоварам.Количество;
				нСтр.КоличествоУпаковок		= ВыборкаПоТоварам.КоличествоУпаковок;
				нСтр.Склад					= ВыборкаПоСкладам.Склад;
				нСтр.НовоеНазначение		= НазначениеЗаказа;
				
				ТелоОписанияЗадачи	= ТелоОписанияЗадачи+""+ВыборкаПоТоварам.Номенклатура+";"+" Зарезирвировано на: "+ВыборкаПоСкладам.Склад+";" + " В количестве: "+ВыборкаПоТоварам.Количество+Символы.ПС;
				//ЗаполнитьЗначенияСвойств(НоваяСтрока,ВыборкаПоТоварам);
				
			КонецЦикла;	// Пока ВыборкаПоТоварам.Следующий() Цикл
			
		КонецЦикла; // Пока РЗ.Следующий() Цикл		

		Попытка
			НовыйДокНазначение.Записать(РежимЗаписиДокумента.Проведение);
			ОписаниеЗадачи	= "Создан документ резервирования к заказу № : " + НовыйДокНазначение.Номер+Символы.ПС+ТелоОписанияЗадачи;
		Исключение
			
			ОписаниеЗадачи	= "Документ резервирования не создан по причине:" + ОписаниеОшибки();
			
		КонецПопытки;
		
	КонецЕсли; // Если РаспределеннаяТаблица.Количество() <> 0 Тогда
			
КонецПроцедуры // Процедура АТОМ_СозданиеПеремещенийДляСозданногоЗаказа(ЗаказКлиента)	

Функция АТОМ_ЗаполнитьРасшифровкуПлатежа(ДокументПБДС,ДокументОснование)
	...
	ТекстЗапроса	= "
		...
		|	ЗаказКлиентаТовары.Ссылка КАК ОснованиеПлатежа,
		|	ЗаказКлиентаТовары.Ссылка КАК Заказ,
		|	ЗаказКлиентаТовары.Ссылка.Партнер КАК Партнер,
		|	ЗаказКлиентаТовары.Ссылка.Валюта КАК ВалютаВзаиморасчетов,
		|	АЭС_ДополнительныеНастройкиИМСрезПоследних.СтатьяДвиженияДенежныхСредств КАК СтатьяДвиженияДенежныхСредств
		|ИЗ
		|	Документ.ЗаказКлиента.Товары КАК ЗаказКлиентаТовары,
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АЭС_ДополнительныеНастройкиИМ.СрезПоследних(&ДатаСреза, ) КАК АЭС_ДополнительныеНастройкиИМСрезПоследних
		|		ПО (ЗаказКлиентаТовары.Ссылка.Организация = АЭС_ДополнительныеНастройкиИМСрезПоследних.Организация)
		|ГДЕ
		...
		|	АЭС_ДополнительныеНастройкиИМСрезПоследних.СтатьяДвиженияДенежныхСредств,
		|	ЗаказКлиентаТовары.Ссылка";
	...
	ДокументПБДС.РасшифровкаПлатежа.Загрузить(Запрос.Выполнить().Выгрузить());
		
КонецФункции // Функция АТОМ_ЗаполнитьРасшифровкуПлатежа(ДокументПБДС)
...
Процедура ЗагрузитьДокументы(ПараметрыОбмена, ТзнДокументов, ДокументXDTO)
	...
	Если Б_ОбменССайтомСервер.ОбъектXDTOСодержитСвойство(ДокументXDTO, "ЗначенияРеквизитов") Тогда
		...
		НайденноеЗначение = НовСтр.ЗначенияРеквизитов.Найти("КПП","Наименование");
		
		Если НайденноеЗначение <> Неопределено Тогда
			
			ОрганизацияКПП 		= НайденноеЗначение.Значение;
			НовСтр.Организация 	= АТОМ_НайтиОрганизацияПоКПП(ОрганизацияКПП);
			
		КонецЕсли; // Если НайденноеЗначение <> Неопределено Тогда
	...
	Если Б_ОбменССайтомСервер.ОбъектXDTOСодержитСвойство(ДокументXDTO, "Склады") Тогда
		...
		Иначе
			...
			иначе
				...
				лСклад 				= ОбработатьИнформациюОСкладеXDTO(ПараметрыОбмена, лСкладXDTO, лНомер);
				гуидСклада			= лСклад.Ид;
				Склад	= АТОМ_ОпределитьСкладВЗаказ(гуидСклада);
				//НовСтр.Склад		= лСклад;
				НовСтр.Склад		= Склад;
			КонецЕсли;	
		КонецЕсли;	
	...
	// + Лактюшин
	Попытка
	лТовары.ЗаполнитьЗначения(НовСтр.Склад.Склад.УчетныйВидЦены, "ВидЦены"); 
	Исключение
	КонецПопытки;
	// - Лактюшин
...
Функция ОбработатьКонтейнеры(ДанныеЗаказов, ПараметрыОбмена)
	...
	ТзнДокументов.Колонки.Добавить("Организация");
...
Функция ОбработатьИнформациюОКонтрагентеXDTO(ПараметрыОбмена, КонтрагентXDTO, НомерДокумента)
	...
	Иначе
		...
		Если ПараметрыОбмена.ИспользоватьФИОДляФизЛиц тогда
			...
			// + Лактюшин
			Если НЕ ЗначениеЗаполнено(СтрИнформацияОКонтрагенте.Фамилия) Тогда
				
				Если Б_ОбменССайтомСервер.ОбъектXDTOСодержитСвойство(КонтрагентXDTO, "Представители") Тогда
					
					лПолноеНаименование = КонтрагентXDTO.Представители.Представитель.Контрагент.Наименование;
					
				КонецЕсли; // Если Б_ОбменССайтомСервер.ОбъектXDTOСодержитСвойство(КонтрагентXDTO, "Представители") Тогда
			...
			Иначе	
				...
				КонецЕсли;	
			КонецЕсли; // Если НЕ ЗначениеЗаполнено(СтрИнформацияОКонтрагенте.Фамилия) Тогда
			// - Лактюшин
...
Функция ОбработатьИнформациюОТабличнойЧастиДокументаXDTO(ПараметрыОбмена, ТзнДокументов, ДокументXDTO)
	...
	// + Лактюшин
	тзнТовары.Колонки.Добавить("ВидТорговогоПредложения");
	тзнТовары.Колонки.Добавить("ВидЦены");
	// - Лактюшин
...
Процедура ОбработатьИнформациюОТовареДокументаXDTO(ПараметрыОбмена, ТоварXDTO, тзнТовары, КодСтроки)
	...
	Иначе
		...
		идПредложения	= НовСтр.Ид;

		НовСтр.ВидТорговогоПредложения	= АТОМ_ПолучитьВидТорговогоПредложения(идПредложения); // Лактюшин ВидПредложения
		...
		Если НЕ Документ.Пустая() тогда
			...
			Если ТекДокумент.НомерВерсии <> Документ.Б_НомерВерсии тогда
				...
				Если НЕ ПараметрыОбмена.НеРедактироватьДокументыПришедшиеССайта тогда
					...
					Если ТекДокумент.ХозОперация = "Заказ товара" И Документ.НеОбновлятьССайта Тогда
						Продолжить;
					Иначе	
...
Процедура СоздатьОбновитьДокумент(ПараметрыОбмена, НазваниеДокумента, ДокументXML, НовыйДокумент, СсылкаНаДокумент = Неопределено)
	...
	Если НовыйДокумент И НЕ ДокументXML.ПометкаУдаления = ИСТИНА тогда
		Если НазваниеДокумента = "ПоступлениеБезналичныхДенежныхСредств" Тогда
			
			Если ПолучитьЗначениеРеквизитаДокумента(ДокументXML.ЗначенияРеквизитов, "Оплачено") = "false" Тогда
				Возврат;	
			Иначе	
				
				Документ = Документы[НазваниеДокумента].СоздатьДокумент();	
				Документ.Организация = ДокументXML.Организация;
				
			КонецЕсли; // Если ПолучитьЗначениеРеквизитаДокумента(ДокументXML.ЗначенияРеквизитов, "Оплачено") = "false" Тогда	
	...
	Для каждого ТекТабЧасть из мТабличныеЧасти Цикл
		...
		Если ТекТабЧасть.ПолеXML = "" тогда
			...
			ПлатежныйДокумент	= Ложь;
			// + Лактюшин 
			Если ТекТабЧасть.ТабличнаяЧасть = "РасшифровкаПлатежа" Тогда
				ПлатежныйДокумент	= Истина;	
				
				ЗаказОснование	= ПолучитьОснование(ДокументXML, "");
				АТОМ_ЗаполнитьРасшифровкуПлатежа(Документ,ЗаказОснование);
				Документ.Организация = ЗаказОснование.Организация;
			...
			Иначе
				...
				Для Каждого ТекРеквизитТЧ из мРеквизитыТЧ Цикл
					...
					Если НЕ ПустаяСтрока(ТекРеквизитТЧ.Значение) тогда
						...
						Если ПлатежныйДокумент И НЕ ЗначениеЗаполнено(Документ.Организация) 
							И ТекРеквизитТЧ.НаименованиеРеквизита1С = "ОснованиеПлатежа" И ЗначениеЗаполнено(НовСтрока[ТекРеквизитТЧ.НаименованиеРеквизита1С]) Тогда
							Документ.Организация = НовСтрока.ОснованиеПлатежа.Организация;
						КонецЕсли;	
			...
			КонецЕсли; // Если ТекТабЧасть.ТабличнаяЧасть = "РасшифровкаПлатежа" Тогда	
			// - Лактюшин
...
#Область ОсобенностиПКО
			...
			ИначеЕсли НазваниеДокумента = "ПоступлениеБезналичныхДенежныхСредств" Тогда // Лактюшин
								
				Документ.ПроведеноБанком		= Истина;
				
				Документ.БанковскийСчет				= АТОМ_ПолучитьБанковскийСчетОрганизацииНаДату(Документ.Дата,Документ.Организация);
				Документ.НомерВходящегоДокумента	= "б/н";
				Попытка
					
					xmlДатаОплаты	= ПолучитьЗначениеРеквизитаДокумента(ДокументXML.ЗначенияРеквизитов, "Дата оплаты");
				
					ДатаОплаты	= XMLЗначение(Тип("Дата"),xmlДатаОплаты);

					Документ.ДатаПроведенияБанком 	= ДатаОплаты;
					Документ.ДатаВходящегоДокумента	= ДатаОплаты;
					Документ.ПроведеноБанком		= Истина;	
				Исключение
					
					Документ.ДатаПроведенияБанком 	= Документ.Дата;
					Документ.ДатаВходящегоДокумента	= Документ.Дата;
				КонецПопытки;
...
#Область ОсобенностиЗаказовИОтгрузок
	Если НазваниеДокумента = "ЗаказКлиента" или НазваниеДокумента = "РеализацияТоваровУслуг" тогда
		...
		СтатусДокумента	= ДокументXML.ЗначенияРеквизитов.Найти("Статуса заказа ИД","Наименование");
		
		Если ЗначениеЗаполнено(СтатусДокумента) Тогда
				
			Если СтатусДокумента.Значение	= "N" Тогда	
				// Принят, ожидается отплата
				Документ.Статус	= Перечисления.СтатусыЗаказовКлиентов.НеСогласован;
				
			ИначеЕсли СтатусДокумента.Значение	= "Y" Тогда	
				// Оплачен, ожидается доставка
				Документ.Статус	= Перечисления.СтатусыЗаказовКлиентов.КОтгрузке;
			
			ИначеЕсли СтатусДокумента.Значение	= "F" Тогда 
				// Выполнен 
				Если Документ.Статус = Перечисления.СтатусыЗаказовКлиентов.КОбеспечению Тогда
			...
			КонецЕсли; // Если СтатусДокумента.Значение	= "F" Тогда
			
		КонецЕсли; // Если ЗначениеЗаполнено(СтатусДокумента) Тогда
		
		// Лактюшин Удалить
		Если НЕ ЗначениеЗаполнено(Документ.Организация) Тогда
			
			ТоварыДокументXML	= ДокументXML.Товары;	
			
			Если ТоварыДокументXML.Количество() <> 0 Тогда 
					
				идПредложения	= ТоварыДокументXML[0].Ид;
					
				Документ.Организация 	= АТОМ_ПолучитьОрганизациюИзТорговогоПредложения(идПредложения);
						
			КонецЕсли; // Если лТовары.Количество() <> 0 Тогда

		КонецЕсли; // Если НЕ ЗначениеЗаполнено(Документ.Организация) Тогда	
		
		Документ.Соглашение	= АТОМ_ПолучитьСоглашениеПоОрганизации(Документ.Организация,Документ.Дата);
		Попытка
			Документ.Менеджер	= Документ.Соглашение.Менеджер;		
		Исключение
		
		КонецПопытки;
		Если НЕ ЗначениеЗаполнено(Документ.Склад) Тогда
			
			Если ТипЗнч(ДокументXML.Склад) = Тип("СправочникСсылка.Склады") Тогда
				СкладВДокмент	= ДокументXML.Склад;	
			Иначе
				Попытка
				СкладВДокмент	= ДокументXML.Склад.Склад;
				Исключение
				КонецПопытки;
			КонецЕсли;	
			Документ.Склад	= СкладВДокмент;
			Для Каждого стр Из Документ.Товары Цикл
				стр.Склад	= СкладВДокмент;
			КонецЦикла;	
			
		КонецЕсли; // Если НЕ ЗначениеЗаполнено(Документ.Склад) Тогда	
...
#КонецОбласти
	...
	Если ДокументСозданИзменен тогда 
		...
		ИначеЕсли НазваниеДокумента = "ЗаказКлиента" тогда 
			...
			ОписаниеЗадачи = "";
			АТОМ_СозданиеПеремещенийДляСозданногоЗаказа(Документ.Ссылка,ОписаниеЗадачи);
			
			ПараметрыЗадачи	= Новый Структура;
			ПараметрыЗадачи.Вставить("Склад",Документ.Склад);
			ПараметрыЗадачи.Вставить("Заказ",Документ.Ссылка);
			ПараметрыЗадачи.Вставить("ОписаниеЗадачи",ОписаниеЗадачи);	
			
			АТОМ_СоздатьЗадачуНаОбработкуЗаказа(ПараметрыЗадачи);
...
Функция ПолучитьЗначениеРеквизитаДокументаПоСоответствию(ПараметрыОбмена, ДокументXML, Документ, ОсновныеНастройкиДокумента, ТекущийРеквизит, СтрокаXML = Неопределено, СтрокаТабличнойЧасти = Неопределено, СтрараяТабличнаяЧасть = Неопределено)
	...
	Если ТекущийРеквизит.ТипДанных = "Из поля XML документа" тогда
		...
		ИначеЕсли ТекущийРеквизит.Значение = "Единица измерения" тогда
			...
			КонецЕсли;
		ИначеЕсли ТекущийРеквизит.Значение = "Вид торгового предложения" тогда
			...
			//	Результат = Справочники.УпаковкиЕдиницыИзмерения.ПустаяСсылка();	
			//Иначе
				Результат = СтрокаXML.ВидТорговогоПредложения;	
			//КонецЕсли;
...
Функция ПолучитьСклад(ПараметрыОбмена, Документ, ДокументXML)
	...
	Иначе
		//лСклад	= АТОМ_ПолучитьСкладИзПунктовВыдачи(идПунктаВыдачи)	АТОМ	
	КонецЕсли;
...
Функция ПолучитьИнформациюОНоменклатуреДляДокумента(ПараметрыОбмена, СтрокаТовара)
	...
	// + Лактюшин. Не создаем характеристики при загрузке 
...
Функция ПолучитьКонтрагента(ПараметрыОбмена, ЮрФизЛицо, ДокументXML)
	...
	// АТОМ Лактюшин +
	Если НЕ ЗначениеЗаполнено(ИнформацияОКонтрагенте.Наименование) Тогда
		Контрагент.Наименование			= ИнформацияОКонтрагенте.Ид;		
	Иначе	
...
Функция ПолучитьОбновитьПартнера(ПараметрыОбмена, Контрагент, ИнформацияОКонтрагенте)
	...
	// АТОМ Лактюшин +
	Если ИнформацияОКонтрагенте.Наименование <> ""  Тогда
		ПартнерОбъект.Наименование			= ИнформацияОКонтрагенте.Ид;
		ПартнерОбъект.НаименованиеПолное	= ИнформацияОКонтрагенте.Ид;
	ИначеЕсли ИнформацияОКонтрагенте.КонтактныеЛица <> Неопределено Тогда 
		Если ИнформацияОКонтрагенте.КонтактныеЛица.Количество() > 0 Тогда	
		
			КонтактноеЛицо	= ИнформацияОКонтрагенте.КонтактныеЛица[0];	
			
			ПартнерОбъект.Наименование			= КонтактноеЛицо.Наименование;
			ПартнерОбъект.НаименованиеПолное	= КонтактноеЛицо.Наименование;
		КонецЕсли;
	Иначе	
		ПартнерОбъект.Наименование			= ИнформацияОКонтрагенте.Наименование;
		ПартнерОбъект.НаименованиеПолное	= ИнформацияОКонтрагенте.Наименование;
