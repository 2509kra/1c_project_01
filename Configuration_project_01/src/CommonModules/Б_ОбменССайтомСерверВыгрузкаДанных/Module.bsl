...
Функция РазборМассиваДанныхПоТипамОбъектов(МассивДанных)
	...
	// + Лактюшин. Подмена складов складами в настройках
	Запрос = Новый Запрос;
	ТекстЗапроса	= "
		|ВЫБРАТЬ
		...
		|ОБЪЕДИНИТЬ ВСЕ
		|ВЫБРАТЬ 
		|	АЭС_ТоварныеПредложения.Склад КАК Склад
		|ИЗ
		|	Документ.АЭС_ТоварныеПредложения КАК АЭС_ТоварныеПредложения
		|ГДЕ
		|	АЭС_ТоварныеПредложения.ТипОперации <> ЗНАЧЕНИЕ(Перечисление.АЭС_ВидТорговогоПредложения.Атом)
		|	И АЭС_ТоварныеПредложения.Склад  <> ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)";
	
	Запрос.Текст	= ТекстЗапроса;
	
	РЗ_Склады	= Запрос.Выполнить().Выбрать();  
	// - Лактюшин
	...
	// + Лактюшин
	Пока РЗ_Склады.Следующий() Цикл
		СпкСклады.Добавить(РЗ_Склады.Склад);	
	КонецЦикла; // Пока РЗ_Склады.Следующий() Цикл
	// - Лактюшин
	
	Для каждого ТекСтр Из МассивДанных Цикл
		Если ТекСтр.ТипОбъекта = "Номенклатура" тогда
			СпкНоменклатура.Добавить(ТекСтр.Объект);	
		//-- Лактюшин ИначеЕсли ТекСтр.ТипОбъекта = "Склад" тогда 
...
Функция ПолучитьИдентификаторыСкладаДляВыгрузки(ОбъектВыгрузки)
	
	Если ТипЗнч(ОбъектВыгрузки.БизнесРегион) = Тип("СправочникСсылка.Склады") Тогда

		Если ЗначениеЗаполнено(ОбъектВыгрузки.БизнесРегион.БизнесРегион) Тогда
			КодРегион	= СтрЗаменить(Сред(ОбъектВыгрузки.БизнесРегион.БизнесРегион.ЗначениеГеографическогоРегиона,0,3),Символ(160),"");	
			ИдСклада	= СтрЗаменить(КодРегион,",","") + "#"+XMLСтрока(ОбъектВыгрузки.БизнесРегион);
		Иначе
			ИдСклада	= XMLСтрока(ОбъектВыгрузки.БизнесРегион);		
		КонецЕсли; // Если ЗначениеЗаполнено(ВыборкаСкладов.БизнесРегион.БизнесРегион) Тогда	
						
	Иначе

		КодРегион	= СтрЗаменить(Сред(ОбъектВыгрузки.БизнесРегион.ЗначениеГеографическогоРегиона,0,3),Символ(160),"");	
		ИдСклада	= СтрЗаменить(КодРегион,",","") + "#"+XMLСтрока(ОбъектВыгрузки.БизнесРегион);

	КонецЕсли; // Если ТипЗнч(ВыборкаСкладов.БизнесРегион) = Тип("СправочникСсылка.Склады") Тогда
...
Процедура АТОМ_ВыгрузитьСкладыXDTO(ПараметрыОбмена, Владелец, ВладелецТип, МенеджерВременныхТаблиц)
	...
	|ИЗ
	|	ВремСклады КАК ВремСклады";
	ЗапросПоСкладам = Запрос.Выполнить().Выгрузить();
		
	Если ЗапросПоСкладам.Количество() = 0 тогда
		Возврат;
	Иначе
		
		ЗапросПоСкладам.Свернуть("БизнесРегион");
		
		Для Каждого ВыборкаСкладов Из ЗапросПоСкладам Цикл
			
			XDTOСклад 					= ПараметрыОбмена.МояФабрикаXDTO.Создать(XDTOСкладТип); 
			
			XDTOСклад.Ид				= ПолучитьИдентификаторыСкладаДляВыгрузки(ВыборкаСкладов);
						
			XDTOСклад.НомерВерсии		= Б_ОбменССайтомСервер.ПолучитьВерсиюОбъекта(ПараметрыОбмена, ВыборкаСкладов.БизнесРегион.ВерсияДанных, "Складов");
			
			XDTOСклад.ПометкаУдаления	= XMLСтрока(ВыборкаСкладов.БизнесРегион.ПометкаУдаления);
			XDTOСклад.Наименование		= ВыборкаСкладов.БизнесРегион.Наименование;
		...
		Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
		Запрос.Выполнить();
	КонецЕсли; // Если ЗапросПоСкладам.Количество() = 0 тогд	
	...
	Запрос.Текст = "ВЫБРАТЬ *
	|ИЗ
	|	АТОМ_ПунктыВыдачи КАК ПунктыВыдачи";
	ЗапросПоПунктамВыдачи = Запрос.Выполнить().Выгрузить();
	
	Если ЗапросПоПунктамВыдачи.Количество() = 0 тогда
		Возврат;
	Иначе
		
		Для Каждого ПунктВыдачи Из ЗапросПоПунктамВыдачи Цикл
		
			XDTOСклад 					= ПараметрыОбмена.МояФабрикаXDTO.Создать(XDTOСкладТип); 
			
			XDTOСклад.Ид				= ПунктВыдачи.КодРегион + "#"+XMLСтрока(ПунктВыдачи.Ссылка);
			XDTOСклад.НомерВерсии		= Б_ОбменССайтомСервер.ПолучитьВерсиюОбъекта(ПараметрыОбмена, ПунктВыдачи.ВерсияДанных, "Складов");
			
			XDTOСклад.ПометкаУдаления	= XMLСтрока(ПунктВыдачи.ПометкаУдаления);
			XDTOСклад.Наименование		= ПунктВыдачи.Наименование;
		...
		//Запрос = Новый Запрос("УНИЧТОЖИТЬ ПунктыВыдачи");
		//Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
		//Запрос.Выполнить();
...
Процедура ВыгрузитьСоглашенияXDTO(ПараметрыОбмена, Владелец, ВладелецТип, МенеджерВременныхТаблиц)
	...
	Запрос = Новый Запрос("УНИЧТОЖИТЬ ВремВидыЦен");
	Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
	Запрос.Выполнить();
...
Процедура ВыгрузитьОбщуюИнформацию(ПараметрыОбмена, МассивДанных, Операция, СтрокаТаблицыКаталогов, НомерСтроки)
	...
	Если МассивСтрок.Количество() > 0 тогда
		...
		Если ПараметрыОбмена.ВыгружатьОстаткиПоСкладам тогда
			...
			АТОМ_ВыгрузитьСкладыXDTO(ПараметрыОбмена, XDTOКлассификатор, XDTOКлассификаторТип, лМенеджерВременныхТаблиц);
		КонецЕсли;
...
Функция СформироватьВременныеТаблицыПоОбщейИнформации(ПараметрыОбмена, СписокСкладов, СписокСоглашений, СписокЕдиниц)
	...
	|	ВЫБОР КОГДА Склады.Б_Идентификатор = """" ТОГДА 
	|		Склады.Ссылка
	|	ИНАЧЕ 
	|		Склады.Б_Идентификатор
	|	КОНЕЦ КАК Идентификатор,
	|	Склады.ВерсияДанных,
	|	ВЫБОР КОГДА Склады.БизнесРегион <> ЗНАЧЕНИЕ(Справочник.БизнесРегионы.ПустаяСсылка) ТОГДА
	|   	Склады.БизнесРегион
	|	ИНАЧЕ
	|   	Склады.Ссылка	
	...
	|	АЭС_ПунктыВыдачи.КодРегион КАК КодРегион,
	|	АЭС_ПунктыВыдачи.ВерсияДанных КАК ВерсияДанных,
	|	АЭС_ПунктыВыдачи.ПометкаУдаления КАК ПометкаУдаления,
	|	АЭС_ПунктыВыдачи.Наименование КАК Наименование
	|ПОМЕСТИТЬ АТОМ_ПунктыВыдачи
	|ИЗ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ВыборкаЦены.ВидЦены КАК Ссылка,
	|	ВыборкаЦены.ВидЦены.ЦенаВключаетНДС КАК ЦенаВключаетНДС,
	|	ВыборкаЦены.ВидЦены.ВалютаЦены КАК Валюта,
	|	ВыборкаЦены.ВидЦены.ВалютаЦены.Наименование КАК ВалютаКод,
	|	ВыборкаЦены.ВидЦены КАК ИдентификаторСоглашения,
	|	ВыборкаЦены.ВидЦены.Б_Идентификатор КАК ИдентификаторВидаЦены,
	|	ВыборкаЦены.ВидЦены.ВерсияДанных КАК ВерсияДанных,
	|	ВыборкаЦены.ВидЦены.ПометкаУдаления КАК ПометкаУдаления,
	|	ВыборкаЦены.ВидЦены.Наименование КАК Наименование
	|ПОМЕСТИТЬ ВремВидыЦен
	|
	|ИЗ
	|	Документ.АЭС_ТоварныеПредложения КАК ВыборкаЦены
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка
	|;
	|
	...
	Для Пакет = 1 по КоличествоПакетов Цикл
		...
		Если массивСтрок.Количество() > 0 тогда
			...
			Если ПараметрыОбмена.ВыгружатьПредложения тогда
				...
				АТОМ_ВыгрузитьПредложенияXDTO(ПараметрыОбмена, ИмяКаталога, XDTOПакетПредложений, XDTOПакетПредложенийТип, лМенеджерВременныхТаблиц, МассивКартинок, МассивФайлов);
			...
			Если ПараметрыОбмена.ВыгружатьЦены тогда  
				...
				АТОМ_ВыгрузитьПрайсы(ПараметрыОбмена, XDTOПредложения, XDTOПредложенияТип, лМенеджерВременныхТаблиц);
			...
			Если ПараметрыОбмена.ВыгружатьОстатки тогда  
				...
				АТОМ_ВыгрузитьОстаткиXDTO(ПараметрыОбмена, XDTOПредложения, XDTOПредложенияТип, лМенеджерВременныхТаблиц);
				XDTOПакетПредложений.Предложения = XDTOПредложения;
...
Функция СформироватьВременныеТаблицыПоИнформацииОТоварахПакета(ПараметрыОбмена, ИдентификаторКаталога, СписокТоваров, СписокСвойств)
	...
	|	ВЫБОР КОГДА Предложения.ТипОперации <> ЗНАЧЕНИЕ(Перечисление.АЭС_ВидТорговогоПредложения.Атом) ТОГДА 
	|		Предложения.Контрагент		
	|	ИНАЧЕ 
	|		Предложения.Организация
	|	КОНЕЦ КАК идПредложения,
	|	Предложения.Номенклатура КАК Номенклатура,	
	|	Предложения.Контрагент КАК Контрагент,
	|	Предложения.ТипЦены КАК ТипЦены,
	|	Предложения.Организация КАК Организация,
	|	Предложения.ТипОперации КАК ТипОперации,
	|	Цены.Цена КАК Цена
	|ПОМЕСТИТЬ АТОМ_Цены	
	|ИЗ
	|	РегистрСведений.АЭС_ТорговыеПредложения.СрезПоследних КАК Предложения
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныНоменклатуры.СрезПоследних КАК Цены
	|		ПО (Предложения.Номенклатура =  Цены.Номенклатура И Предложения.ТипЦены =  Цены.ВидЦены)
	|ГДЕ 
	|	Цены.Цена > 0		
	|ИНДЕКСИРОВАТЬ ПО
	|		Номенклатура
	...
	|	Номенклатура.Марка КАК Производитель,
	|	Номенклатура.Марка.Наименование КАК ПроизводительНаименование,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ВЫБОР КОГДА АЭС_ТорговыеПредложения.Контрагент <> ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка) ТОГДА
	|		АЭС_ТорговыеПредложения.Контрагент
	|	Иначе
	|		АЭС_ТорговыеПредложения.Организация	
	|	КОНЕЦ КАК идПредложения,
	|		ВЫБОР КОГДА АЭС_ТорговыеПредложения.Склад.БизнесРегион <> ЗНАЧЕНИЕ(Справочник.БизнесРегионы.ПустаяСсылка) ТОГДА
	|			АЭС_ТорговыеПредложения.Склад.БизнесРегион
	|		ИНАЧЕ
	|			АЭС_ТорговыеПредложения.Склад		 
	|		КОНЕЦ КАК  Склад,
	|	АЭС_ТорговыеПредложения.ТипЦены КАК ТипЦены,
	|	АЭС_ТорговыеПредложения.Регистратор.ДеактивироватьТорговоеПредложение КАК Деактивировать,
	|	АЭС_ТорговыеПредложения.СпособДоставки КАК СпособДоставки,
	|	АЭС_ТорговыеПредложения.Номенклатура.ВерсияДанных КАК ВерсияДанныхНоменклатуры,
	|	АЭС_ТорговыеПредложения.Номенклатура КАК Номенклатура
	|ПОМЕСТИТЬ АТОМ_Предложения
	|ИЗ
	|	РегистрСведений.АЭС_ТорговыеПредложения.СрезПоследних(,Номенклатура В (&СписокТоваров)) КАК АЭС_ТорговыеПредложения
	|ИНДЕКСИРОВАТЬ ПО
	|	Номенклатура
	|;
	|////////////////////////////////////////////////////////////////////////////////
	...
	|	НоменклатураСХарактеристиками.Склад КАК Склад,
	...
	|		ВЫБОР КОГДА АЭС_ТорговыеПредложения.Регистратор.ТипОперации = ЗНАЧЕНИЕ(Перечисление.АЭС_ВидТорговогоПредложения.Атом) ТОГДА 
	|			АЭС_ТорговыеПредложения.Регистратор.БизнесРегион
	|		ИНАЧЕ 
	|			АЭС_ТорговыеПредложения.Регистратор.Склад
	|		КОНЕЦ КАК Склад,
	|		ВЫБОР КОГДА АЭС_ТорговыеПредложения.Контрагент <> ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка) ТОГДА 
	|			АЭС_ТорговыеПредложения.Контрагент
	|		ИНАЧЕ 
	...
	|		ВЫБОР КОГДА АЭС_ТорговыеПредложения.Контрагент <> ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка) ТОГДА 
	|			АЭС_ТорговыеПредложения.Контрагент.ВерсияДанных
	|		ИНАЧЕ 
	|			АЭС_ТорговыеПредложения.Организация.ВерсияДанных
	|		КОНЕЦ КАК  ВерсияДанныхХарактеристики,
	...
	|		ЛОЖЬ КАК ПометкаУдаленияХарактеристики,
	...
	|	ИЗ
	|		РегистрСведений.АЭС_ТорговыеПредложения.СрезПоследних КАК АЭС_ТорговыеПредложения
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВремНоменклатура КАК ВремНоменклатура
	|			ПО АЭС_ТорговыеПредложения.Номенклатура = ВремНоменклатура.Номенклатура	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	|		ВремНоменклатура.Номенклатура,
	...
	|	ВремНоменклатураСХарактеристикамиВся.Склад КАК Склад,
	...
	|	АЭС_ОстаткиТоваровКонтрагента.Номенклатура КАК Номенклатура,
	|	АЭС_ОстаткиТоваровКонтрагента.Номенклатура КАК ИдентификаторНоменклатуры,
	|	"""" КАК Характеристика,
	|	"""" КАК ИдентификаторХарактеристики,
	|	АЭС_ОстаткиТоваровКонтрагента.Склад КАК Склад,
	|	АЭС_ОстаткиТоваровКонтрагента.Склад КАК ИдентификаторСклада,
	|	АЭС_ОстаткиТоваровКонтрагента.Количество КАК Остаток
	|ПОМЕСТИТЬ вт_ОстаткиПоставщика
	|ИЗ
	|	РегистрСведений.АЭС_ОстаткиТоваровКонтрагента КАК АЭС_ОстаткиТоваровКонтрагента
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	КОНЕЦ КАК ИдентификаторХарактеристики,
	|	ВЫБОР
	|		КОГДА Остатки.Склад.БизнесРегион <> ЗНАЧЕНИЕ(Справочник.БизнесРегионы.ПустаяСсылка)
	|			ТОГДА Остатки.Склад.БизнесРегион
	|		ИНАЧЕ Остатки.Склад
	|	КОНЕЦ КАК Склад,
	|	ВЫБОР
	...
	|ВЫБРАТЬ
	|	Остатки.Номенклатура КАК Номенклатура,
	|	Остатки.Номенклатура КАК ИдентификаторНоменклатуры,
	|	Остатки.Характеристика КАК Характеристика,
	|	Остатки.ИдентификаторХарактеристики КАК ИдентификаторХарактеристики,
	|	Остатки.Склад КАК Склад,
	|	Остатки.Склад КАК ИдентификаторСклада,
	|	Остатки.Остаток КАК Остаток
	|ПОМЕСТИТЬ ВремОстаткиПоСкладам	
	|ИЗ
	|	(ВЫБРАТЬ
	|		вт_ОстаткиПоставщика.Номенклатура КАК Номенклатура,
	|		вт_ОстаткиПоставщика.ИдентификаторНоменклатуры КАК ИдентификаторНоменклатуры,
	|		вт_ОстаткиПоставщика.Характеристика КАК Характеристика,
	|		вт_ОстаткиПоставщика.ИдентификаторХарактеристики КАК ИдентификаторХарактеристики,
	|		вт_ОстаткиПоставщика.Склад КАК Склад,
	|		вт_ОстаткиПоставщика.ИдентификаторСклада КАК ИдентификаторСклада,
	|		вт_ОстаткиПоставщика.Остаток КАК Остаток
	|	ИЗ
	|		вт_ОстаткиПоставщика КАК вт_ОстаткиПоставщика
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	|		АТОМОстатки.Номенклатура,
	|		АТОМОстатки.ИдентификаторНоменклатуры,
	|		АТОМОстатки.Характеристика,
	|		АТОМОстатки.ИдентификаторХарактеристики,
	|		АТОМОстатки.Склад,
	|		АТОМОстатки.ИдентификаторСклада,
	|		АТОМОстатки.Остаток
	|	ИЗ
	|		вт_АТОМОстатки КАК АТОМОстатки) КАК Остатки
	|
	|ИНДЕКСИРОВАТЬ ПО
...
Процедура АТОМ_ВыгрузитьПредложенияXDTO(ПараметрыОбмена, ИмяКаталога, Владелец, ВладелецТип, лМенеджерВременныхТаблиц, МассивКартинок, МассивФайлов)
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
	               |	*
	               |ИЗ
	               |	АТОМ_Предложения КАК ВремНоменклатураСХарактеристиками
	               |
	               |УПОРЯДОЧИТЬ ПО
	               |	ВремНоменклатураСХарактеристиками.Номенклатура.Наименование";
	...
	Если ТзнПредложений.Количество() > 0 тогда	
		...
		Для каждого ТекПредложение из ТзнПредложений Цикл
			...
			лИд 			= XMLСтрока(ТекПредложение.Номенклатура) + "#" + XMLСтрока(ТекПредложение.идПредложения); 
			
			лНаименование   = Строка(ТекПредложение.идПредложения);
			лВерсияДанных   = ТекПредложение.ВерсияДанныхНоменклатуры;
			
			Если ТекПредложение.Номенклатура.ПометкаУдаления Тогда//ИЛИ ТекПредложение.Деактивировать Тогда
				лПометкаУдаления= XMLСтрока(Истина);	
			Иначе
				лПометкаУдаления= XMLСтрока(Ложь);	
			КонецЕсли; // Если ТекПредложение.Номенклатура.ПометкаУдаления ИЛИ ТекПредложение.Деактивировать Тогда	
				
			//XDTOПредложение.Склад			= XMLСтрока(ТекПредложение.Склад);
			XDTOПредложение.Ид 				= лИд;
			XDTOПредложение.НомерВерсии 	= Б_ОбменССайтомСервер.ПолучитьВерсиюОбъекта(ПараметрыОбмена, лВерсияДанных, "Предложений");
			XDTOПредложение.ПометкаУдаления	= лПометкаУдаления;
			...
			Если ТипЗнч(ТекПредложение.идПредложения) = Тип("СправочникСсылка.Организации") Тогда
				XDTOЗначенияСвойства = ПараметрыОбмена.МояФабрикаXDTO.Создать(XDTOЗначенияСвойстваТип);		
				XDTOЗначенияСвойства.Ид = XMLСтрока("d83f64f7-2469-11e9-80e0-005056b10ac6");
				XDTOЗначенияСвойства.Значение.Добавить(XMLСтрока(Сред(ТекПредложение.идПредложения.КПП,0,2)));				
				XDTOЗначенияСвойств.ЗначенияСвойства.Добавить(XDTOЗначенияСвойства);
			Иначе
...
Процедура ВыгрузитьПрайсы(ПараметрыОбмена, Владелец, ВладелецТип, лМенеджерВременныхТаблиц)
	...
	Если ТзнЦен.Количество() > 0 тогда
		...
		Для НомерСтроки = 0 по  КоличествоСтрок Цикл
			...
			Иначе
				ЦенаТовара = 0; 
				Б_ОбменССайтомСервер.СообщитьПодробно("У товара:" + Строка(ТекЦенаСлед.ИдентификаторНоменклатуры) + " Коэффициент = 0, поэтому будет установлена цена = 0", ПараметрыОбмена);	
			КонецЕсли;
...
Процедура АТОМ_ВыгрузитьПрайсы(ПараметрыОбмена, Владелец, ВладелецТип, лМенеджерВременныхТаблиц)
Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = лМенеджерВременныхТаблиц;
	//Временная таблица формируется в функции СформироватьВременныеТаблицыПоИнформацииОТоварахПакета
	Запрос.Текст = "ВЫБРАТЬ *
	|ИЗ
	|	АТОМ_Цены КАК ВремЦены
	|
	|УПОРЯДОЧИТЬ ПО
	|	Номенклатура";
	...
	Если ТзнЦен.Количество() > 0 тогда
		...
		Для НомерСтроки = 0 по  КоличествоСтрок Цикл
			...
			ТекЦена = ТзнЦен[НомерСтроки];
			Если НЕ ЗначениеЗаполнено(ТекЦена.идПредложения) Тогда
				Продолжить;
			КонецЕсли;
			...
			//Иначе 
				...
				Единица 	= ТекЦена.Номенклатура.ЕдиницаИзмерения;  // Исправить
				Коэффициент = 1;
				
		//	КонецЕсли;                      
			
			ИдПредложения 		= ?(ЗначениеЗаполнено(ТекЦена.идПредложения), XMLСтрока(ТекЦена.Номенклатура) + "#" + XMLСтрока(ТекЦена.идПредложения), XMLСтрока(ТекЦена.Номенклатура));
			...
			ПредставлениеЦены 	= СокрЛП(ТекЦена.Цена) + " " + СокрЛП(ТекЦена.ТипЦены.ВалютаЦены) + " за " + СокрЛП(Единица);

			XDTOЦена.Представление 		= ПредставлениеЦены;
			XDTOЦена.ИдТипаЦены			= XMLСтрока(ТекЦена.ТипЦены);
			XDTOЦена.ЦенаЗаЕдиницу 		= Формат(ЦенаТовара, ПараметрыОбмена.ФорматСуммы);            
			XDTOЦена.Валюта       		= ?(ЗначениеЗаполнено(ТекЦена.ТипЦены.ВалютаЦены), Лев(ТекЦена.ТипЦены.ВалютаЦены, 3), "");
			//XDTOЦена.ИдЕдиницыИзмерения = XMLСтрока(ТекЦена.БазоваяЕдиницаИзмеренияИдентификатор);
		
			XDTOНалог				= ПараметрыОбмена.МояФабрикаXDTO.Создать(XDTOНалогТип);
			XDTOНалог.Наименование 	= ПараметрыОбмена.НаименованиеНалога;
			XDTOНалог.УчтеноВСумме 	= XMLСтрока(ТекЦена.ТипЦены.ЦенаВключаетНДС);
		
			XDTOЦена.Налог.Добавить(XDTOНалог);
		
			
			Если ТекЦена.Цена > 0 И ТзнЦен[НомерСтроки].идПредложения = ТекЦена.идПредложения тогда
				XDTOЦены.Цена.Добавить(XDTOЦена);
			КонецЕсли;
...
Процедура АТОМ_ВыгрузитьОстаткиXDTO(ПараметрыОбмена, Владелец, ВладелецТип, лМенеджерВременныхТаблиц)
	...
	|	ВремСклады КАК ВремСклады";
	ТзнСкладов = ЗапросПоСкладам.Выполнить().Выгрузить();
	ТзнСкладов.Свернуть("БизнесРегион");	
	...
	Пока Выборка.Следующий() Цикл
		...
		ЗапросПоОстаткам.Текст = "ВЫБРАТЬ * ИЗ ВремОстаткиПоСкладам КАК ВремОстаткиПоСкладам
		|ГДЕ 
		|ВремОстаткиПоСкладам.Номенклатура = &Номенклатура  
		|УПОРЯДОЧИТЬ ПО
		|	ИдентификаторНоменклатуры";
		ЗапросПоОстаткам.УстановитьПараметр("Номенклатура",Выборка.Номенклатура);
		
		ТзнОстатков = ЗапросПоОстаткам.Выполнить().Выгрузить();
		ТзнОстатков.Свернуть("ИдентификаторНоменклатуры,ИдентификаторХарактеристики,Номенклатура,Склад,Характеристика","Остаток");
		...
		ИдПредложения 		= ?(ЗначениеЗаполнено(Выборка.ИдентификаторХарактеристики), XMLСтрока(Выборка.Номенклатура) + "#" + XMLСтрока(Выборка.ИдентификаторХарактеристики), XMLСтрока(Выборка.ИдентификаторНоменклатуры));
		...
		Если лВыгрузкаПоСкладам тогда
			...
			Для Каждого ТекСклад Из ТзнСкладов Цикл 
				...
				НайденнаяСтрока = ТзнОстатков.Найти(ТекСклад.БизнесРегион);

				XDTOОстаток					= ПараметрыОбмена.МояФабрикаXDTO.Создать(XDTOОстатокТип); 

				Если НайденнаяСтрока = Неопределено ИЛИ ТекСклад.БизнесРегион <> Выборка.Склад Тогда
					
					XDTOСклад				= ПараметрыОбмена.МояФабрикаXDTO.Создать(XDTOСкладТип);	
					XDTOСклад.Ид			= ПолучитьИдентификаторыСкладаДляВыгрузки(ТекСклад);
					XDTOСклад.Количество	= 0;
				...
				Иначе
					...
					XDTOСклад.Ид			= ПолучитьИдентификаторыСкладаДляВыгрузки(ТекСклад);
					XDTOСклад.Количество	= НайденнаяСтрока.Остаток;
					//ИтогоОстаток     		= ИтогоОстаток+НайденнаяСтрока.Остаток;
					XDTOОстаток.Склад		= XDTOСклад;			
										
				КонецЕсли; // Если НайденнаяСтрока = Неопределено ИЛИ ТекСклад.БизнесРегион <> Выборка.Склад Тогда	
		...
		XDTOПредложение.Остатки = XDTOОстатки;
    	//XDTOПредложение.Количество	= ИтогоОстаток;
		Владелец.Предложение.Добавить(XDTOПредложение);
...
Функция СформироватьВременныеТаблицыПоДокументам(ПараметрыОбмена, МассивДанных)
	...
	//|;
	|ВЫБРАТЬ
	|	АтомОтгрузка.Отгрузка КАК Отгрузка,
	|	АтомОтгрузка.Заказ КАК Заказ,
	|	АтомОтгрузка.Отгрузка.Дата КАК Дата,
	|	АтомОтгрузка.Отгрузка.Номер КАК Номер
	|ПОМЕСТИТЬ ВремОтгрузкиЗаказов
	|ИЗ
	|	(ВЫБРАТЬ
	|		ВложенныйЗапрос.Регистратор КАК Отгрузка,
	...
	|		ВложенныйЗапрос.Регистратор.Номер
	|	ИЗ
	|		(ВЫБРАТЬ
	|			ВремЗаказыСВалютой.Валюта КАК Валюта,
	|			ВремЗаказыСВалютой.Заказ КАК Заказ,
	|			МАКСИМУМ(Оформлено.Ссылка) КАК Регистратор
	|		ИЗ
	|			ВремЗаказыСВалютой КАК ВремЗаказыСВалютой
	|				ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.АЭС_Платежи КАК Оформлено
	|				ПО ВремЗаказыСВалютой.Заказ = Оформлено.Заказ
	|					И (Оформлено.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЧековККМ.Пробит))
	|					И (Оформлено.Проведен)
	|		
	|		СГРУППИРОВАТЬ ПО
	|			ВремЗаказыСВалютой.Валюта,
	|			ВремЗаказыСВалютой.Заказ) КАК ВложенныйЗапрос) КАК АтомОтгрузка
	|;
	// - Атом
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|	ВремИнформацияОЗаказахПромежуточная.Перевозчик,
	...
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ДокументыОтгрузки.Ссылка КАК Документ,
	...
	|       	""Отгрузка"" КАК ТипДокумента
	|		ИЗ 		
	|			Документ.АЭС_Платежи КАК Платежи
	|		ГДЕ 
	|			Платежи.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЧековККМ.Пробит)
	|           И Платежи.Проведен
	| 			И Платежи.Заказ В (ВЫБРАТЬ Объект ИЗ ВремМассивДанных)
	...
	//|	Отгрузка
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ДокументыОтгрузки.Отгрузка КАК Отгрузка,
	|   ДокументыОтгрузки.Объект КАК Объект,
	|	ДокументыОтгрузки.Валюта КАК Валюта,
	|	ДокументыОтгрузки.Б_Идентификатор КАК Б_Идентификатор,
	|	ДокументыОтгрузки.ВерсияДанных КАК ВерсияДанных,
	|	ДокументыОтгрузки.ПометкаУдаления КАК ПометкаУдаления,
	|	ДокументыОтгрузки.Номер КАК Номер,
	|	ДокументыОтгрузки.Дата КАК Дата,
	|	ДокументыОтгрузки.Проведен КАК Проведен,
	|	ДокументыОтгрузки.АдресДоставки КАК АдресДоставки,
	|	ДокументыОтгрузки.Контрагент КАК Контрагент,
	|	ДокументыОтгрузки.Партнер КАК Партнер,
	|	ДокументыОтгрузки.Склад КАК Склад,
	|	ДокументыОтгрузки.Комментарий КАК Комментарий,
	|	ДокументыОтгрузки.ЦенаВключаетНДС КАК ЦенаВключаетНДС,
	|	ДокументыОтгрузки.НалогообложениеНДС КАК НалогообложениеНДС,
	|	ДокументыОтгрузки.Статус КАК Статус,
	|	ДокументыОтгрузки.БанковскийСчетКонтрагента КАК БанковскийСчетКонтрагента,
	|	ДокументыОтгрузки.Б_ДатаДокумента КАК Б_ДатаДокумента,
	|	ДокументыОтгрузки.ЗаказКлиента КАК ЗаказКлиента,
	|	ДокументыОтгрузки.СуммаДокумента КАК СуммаДокумента
	|ПОМЕСТИТЬ ВремВыгружаемыеОтгрузки
	|ИЗ
	|(
	|	ВЫБРАТЬ 
	...
	|ОБЪЕДИНИТЬ ВСЕ
	|ВЫБРАТЬ
	|	Платежи.Ссылка КАК Отгрузка,
	|	Платежи.Ссылка КАК Объект,
	|	Платежи.Валюта КАК Валюта,
	|	Платежи.Ссылка КАК Б_Идентификатор,
	|	Платежи.ВерсияДанных КАК ВерсияДанных,
	|	Платежи.ПометкаУдаления КАК ПометкаУдаления,
	|	Платежи.Номер КАК Номер,
	|	Платежи.Дата КАК Дата,
	|	Платежи.Проведен КАК Проведен,
	|	"""" КАК АдресДоставки,
	|	Платежи.Контрагент КАК Контрагент,
	|	Платежи.Контрагент.Партнер КАК Партнер,
	|	Платежи.Склад КАК Склад,
	|	"""" КАК Комментарий,
	|	Платежи.ЦенаВключаетНДС КАК ЦенаВключаетНДС,
	|	Платежи.НалогообложениеНДС КАК НалогообложениеНДС,
	|	ЗНАЧЕНИЕ(Перечисление.СтатусыРеализацийТоваровУслуг.Отгружено) КАК Статус,
	|	"""" КАК БанковскийСчетКонтрагента,
	|	Платежи.Дата КАК Б_ДатаДокумента,
	|	Платежи.Заказ КАК ЗаказКлиента,
	|	Платежи.СуммаДокумента КАК СуммаДокумента
	|ИЗ
	|	Документ.АЭС_Платежи КАК Платежи
	|ГДЕ
	|	Платежи.Ссылка В
	...
	Если КоличествоПакетов > 0 тогда
		...
		Для Пакет = 1 по КоличествоПакетов Цикл
			...
			Для каждого ТекДокумент из НайденныеДокументы Цикл
				...
				Если ТекДокумент.ТипОбъекта = "Заказ" И ТекДокумент.Объект.ПометкаУдаления Тогда
					...
					Соединение = Новый HTTPСоединение("market.atomsbt.ru", 443,"user","bitrix",,,Новый ЗащищенноеСоединениеOpenSSL,);

					НомерЗаказа = ТекДокумент.Объект.НомерПоДаннымКлиента;
						//
					Запрос = Новый HTTPЗапрос("local/classes/orders/index.php?ORDER_ID="+НомерЗаказа+"&ORDER_STATUS=CN");
...
Функция ВыгрузитьЗаказXDTO(ПараметрыОбмена, МенеджерВременныхТаблиц, Документ, КоличествоЗаказов)
	...
	Если ЗначениеЗаполнено(Документ.НомерДокументаОтгрузки) Тогда
...
Функция ВыгрузитьОтгрузкуXDTO(ПараметрыОбмена, МенеджерВременныхТаблиц, Документ, КоличествоОтгрузок)
	...
	Если ТипЗнч(Документ.Объект) = Тип("ДокументСсылка.АЭС_Платежи") Тогда
	...
	Если ТипЗнч(Документ.Объект) = Тип("ДокументСсылка.АЭС_Платежи") Тогда
		...
		ЗапросПоТоварам.УстановитьПараметр("Документ", Документ.Заказ);	
...
//			лТипНоменклатуры = "Товар";
		...
		атом_ИДТовара = "";
		// - Атом
		ДобавитьСвойстваПозицийТовараДокумента(ПараметрыОбмена, Документ, МенеджерВременныхТаблиц, XDTOЗначенияРеквизитов, XDTOЗначениеРеквизитаТип, ТекНоменклатура,атом_ИДТовара);
		
		// + Атом
		Если ЗначениеЗаполнено(атом_ИДТовара) Тогда
			
			XDTOТовар.Ид 			= атом_ИДТовара;
...
Процедура ДобавитьСвойстваПозицийТовараДокумента(ПараметрыОбмена, ИнформацияОДокументе, МенеджерВременныхТаблиц, XDTOЗначенияРеквизитов, XDTOЗначениеРеквизитаТип, ПозицияТовара,атом_ИДТовара);
	...
	Если ТипЗнч(ИнформацияОДокументе.Объект) = Тип("ДокументСсылка.АЭС_Платежи") Тогда
		
		Запрос.УстановитьПараметр("Документ"	, ИнформацияОДокументе.Заказ);
	...
	Пока Выборка.Следующий() Цикл
		...
		// + Атом
		Если Выборка.НаименованиеСвойства = "PRODUCT.XML_ID" Тогда
			атом_ИДТовара = Выборка.Значение; 	
		КонецЕсли;
		// - Атом
