...
Функция СоздатьДокументВыемкаДенежныхСредствИзКассыККМ(ВходныеДанные) Экспорт
	...
	НовыйДокумент.СуммаДокумента = ВходныеДанные.СуммаДокумента;
	НовыйДокумент.Кассир         = Пользователи.ТекущийПользователь();
	НовыйДокумент.АгентскийПлатеж= ВходныеДанные.АгентскийПлатеж;
...
Функция ЗаполнитьОтчетОРозничныхПродажах(ОтчетОРозничныхПродажахОбъект, КассоваяСмена, ОписаниеОшибки = "") Экспорт
	...
	Попытка
		...
		|	ВЫБОР
		|		КОГДА ЧекККМТовары.Ссылка.АгентскийПлатеж = ИСТИНА
		...
		|	ЧекККМТовары.СуммаАг КАК СуммаАг
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	СписокТоваров.Коррекция КАК Коррекция,
		...
		|	СписокТоваров.Организация КАК Организация,
		|	СписокТоваров.НалогообложениеНДС КАК НалогообложениеНДС,
		|	СУММА(СписокТоваров.СуммаАг) КАК СуммаАг
		|ИЗ
		|	(ВЫБРАТЬ
		|		СписокТоваров.НоменклатураНабора КАК НоменклатураНабора,
		|		СписокТоваров.ХарактеристикаНабора КАК ХарактеристикаНабора,
		...
		|		СписокТоваров.Организация КАК Организация,
		|		СписокТоваров.НалогообложениеНДС КАК НалогообложениеНДС,
		|		СписокТоваров.СуммаАг КАК СуммаАг
		|	ИЗ
		...
		|	ИндексыПоАналитике.ИндексПоАналитике КАК ИндексПоАналитике,
		|	ВЫБОР
		|		КОГДА ОплатаПлатежнымиКартами.АгентскийПлатеж
		...
		|		ЧекККМОплатаПлатежнымиКартами.Ссылка.АгентскийПлатеж КАК АгентскийПлатеж
		|	ИЗ
		...
		|		ЧекККМВозвратОплатаПлатежнымиКартами.Ссылка.АгентскийПлатеж
		|	ИЗ
		...
		|		ЧекККМСерии.Ссылка.КассаККМ,
		|		ЧекККМСерии.Ссылка.Организация,
		|		ЧекККМСерии.Ссылка.Склад,
		|		ЧекККМСерии.Ссылка.Валюта,
		|		ЧекККМСерии.Ссылка.ВидЦены
		|	ИЗ
		|		Документ.ЧекККМ.Серии КАК ЧекККМСерии
		|	ГДЕ
		|		ЧекККМСерии.Ссылка В
		...
		|		ЧекККМКоррекцииСерии.Ссылка.КассаККМ,
		|		ЧекККМКоррекцииСерии.Ссылка.Организация,
		|		ЧекККМКоррекцииСерии.Ссылка.Склад,
		|		ЧекККМКоррекцииСерии.Ссылка.Валюта,
		|		ЧекККМКоррекцииСерии.Ссылка.ВидЦены
		|	ИЗ
		...
		|					ЧекиККМ КАК ЧекиККМ
		|				ГДЕ
		|					ЧекиККМ.Коррекция = ИСТИНА)) КАК Серии
		|		ЛЕВОЕ СОЕДИНЕНИЕ ИндексыПоАналитике КАК ИндексыПоАналитике
		...
		|	
		|	ВЫБРАТЬ
		|		БонусныеБаллы.Ссылка.КассаККМ,
		|		БонусныеБаллы.Ссылка.Организация,
		|		БонусныеБаллы.Ссылка.Склад,
		|		БонусныеБаллы.Ссылка.Валюта,
		|		БонусныеБаллы.Ссылка.ВидЦены,
		|		БонусныеБаллы.Ссылка.Партнер,
		...
		|					ЧекиККМ.Ссылка
		|				ИЗ
		|					ЧекиККМ КАК ЧекиККМ)) КАК БонусныеБаллы
		|		ЛЕВОЕ СОЕДИНЕНИЕ ИндексыПоАналитике КАК ИндексыПоАналитике
		...
		|		Документ.ЧекККМВозврат КАК ЧекККМВозврат
		|	ГДЕ
		|		ЧекККМВозврат.БонусныеБаллы.Ссылка В
		|				(ВЫБРАТЬ
		|					ЧекиККМ.Ссылка
		|				ИЗ
		|					ЧекиККМ КАК ЧекиККМ)) КАК БонусныеБаллы
		...
		|	
		|	ВЫБРАТЬ
		|		Таблица.Ссылка.КассаККМ,
		|		Таблица.Ссылка.Организация,
		|		Таблица.Ссылка.Склад,
		|		Таблица.Ссылка.Валюта,
		|		Таблица.Ссылка.ВидЦены,
		|		Таблица.Справка2,
		...
		|					ЧекиККМ.Ссылка
		|				ИЗ
		|					ЧекиККМ КАК ЧекиККМ)) КАК АкцизныеМарки
		|		ЛЕВОЕ СОЕДИНЕНИЕ ИндексыПоАналитике КАК ИндексыПоАналитике
		...
		Пока ВыборкаПоАналитике.Следующий() Цикл
			...
			ОтчетОРозничныхПродажахОбъект.СуммаДокументаАг= ОтчетОРозничныхПродажахОбъект.Товары.Итог("СуммаАг") - ОтчетОРозничныхПродажахОбъект.ОплатаПлатежнымиКартами.Итог("СуммаАг");
