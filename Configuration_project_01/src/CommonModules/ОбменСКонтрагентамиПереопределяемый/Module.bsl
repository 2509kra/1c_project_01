	...
	Если НайденСледующий Тогда
		...
		Пока СтрокаТовары.Следующий() Цикл
			...
			Если ТипЗнч(СтрокаТовары.Ссылка) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
				Если ЗначениеЗаполнено(СтрокаТовары.Ссылка.ВидыЗапасов) Тогда 	
					Если СтрокаТовары.НомерСтроки > 0 Тогда
						СтрокаТабЧасти = СтрокаТовары.Ссылка.ВидыЗапасов[СтрокаТовары.НомерСтроки - 1];
					Иначе
						СтрокаТабЧасти = СтрокаТовары.Ссылка.ВидыЗапасов[СтрокаТовары.НомерСтроки];
					КонецЕсли;
				Иначе
					Если СтрокаТовары.НомерСтроки > 0 Тогда
