...
Функция СформироватьПечатнуюФормуКО2(МассивОбъектов, ОбъектыПечати, ПараметрыПечати)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.Ссылка.АгентскийПлатеж И Выборка.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыдачаДенежныхСредствВДругуюКассу Тогда
			ОбластьМакета.Параметры.ДебетСубсчет = "50.04.1";	//АМА
		Иначе
