...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	//АЭС МИС номер формируется по другим правилам
	...
	Если Не ЗначениеЗаполнено(Номер) Тогда
		...
		ЭтотОбъект.ПредставлениеНомера =  Документы.СчетФактураВыданный.ПолучитьПредставлениеНомера(ЭтотОбъект);
	КонецЕсли;
...
Функция КодВидаОперации(Основание = Неопределено) Экспорт
	...
	Если Выборка.Следующий() Тогда
		...
		Иначе
			//АЭС МИС (
			//КодВидаОперации = "01";
			Если Основание.Контрагент.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ФизЛицо Тогда
				КодВидаОперации = "26";
			Иначе
				КодВидаОперации = "01";
			КонецЕсли;
