	...
	Для Каждого СтруктураОбъектов Из СтруктураТипов Цикл
		...
		Для Каждого ДокументОснование Из СтруктураОбъектов.Значение Цикл
			...
			Если НЕ ДанныеДляПечати = Неопределено Тогда
				ЗаполнитьТабличныйДокументИНВ3_АЭС(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати, ДокументОснование);
			КонецЕсли;
...
Процедура ЗаполнитьТабличныйДокументИНВ3_АЭС(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати, ИнвентаризационнаяОпись)
	...
	Макет = УправлениеПечатью.МакетПечатнойФормы("Обработка.ПечатьИНВ3.ПФ_MXL_ИНВ3_АЭС");
	...
	ОбластьПодвалСтраницы = Макет.ПолучитьОбласть("ПодвалСтраницы");
	ОбластьПодвалОписи = Макет.ПолучитьОбласть("ПодвалОписи");
	ОбластьПодвалОписи1 = Макет.ПолучитьОбласть("ПодвалОписи1");
	ОбластьПодвалОписиЧленыКомисии = Макет.ПолучитьОбласть("ОбластьЧленКомисси");
	
	ОбластьЗаголовок.Параметры.Заполнить(Шапка);
	ОбластьЗаголовок.Параметры.ФормаСобственностиЦенностей = НСтр("ru='в собственности организации'");
	ОбластьЗаголовок.Параметры.ДолжностьМОЛ1  = Шапка.ДолжностьКладовщика;
	ОбластьЗаголовок.Параметры.ФИОМОЛ1        = ФизическиеЛицаУТ.ФамилияИнициалыФизЛица(Шапка.Кладовщик, Шапка.ДатаДокумента);
	ОбластьЗаголовок.Параметры.НомерДокумента = ИнвентаризационнаяОпись.НомерПриказа;//ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(Шапка.НомерДокумента);
	ОбластьЗаголовок.Параметры.ДатаДокумента  = Формат(ИнвентаризационнаяОпись.ДатаПриказа,"ДФ=dd.MM.yyyy");
	ОбластьЗаголовок.Параметры.ОснованиеНомер = ИнвентаризационнаяОпись.НомерПриказа;
	ОбластьЗаголовок.Параметры.ОснованиеДата  = Формат(ИнвентаризационнаяОпись.ДатаПриказа,"ДФ=dd.MM.yyyy");
	...
	Комиссия = ИнвентаризационнаяОпись.Комиссия.Выгрузить();
	Комиссия.Сортировать("Председатель Убыв");
	ЧленКомиссии = 1;
	Для Каждого СТЧ Из Комиссия Цикл 
		Если ЧленКомиссии<= 3 Тогда 
			ОбластьПодвалОписи.Параметры["ДолжностьЧленаКомиссии" + ЧленКомиссии] = СТЧ.Должность;
			ОбластьПодвалОписи.Параметры["ФИОЧленаКомиссии" + ЧленКомиссии] = СТЧ.Пользователи;
			Если ЧленКомиссии = 3 Тогда
				ТабличныйДокумент.Вывести(ОбластьПодвалОписи);
			КонецЕсли;
			ЧленКомиссии = ЧленКомиссии +1;
		Иначе
			ОбластьПодвалОписиЧленыКомисии.Параметры["ДолжностьЧленаКомиссии"] = СТЧ.Должность;
			ОбластьПодвалОписиЧленыКомисии.Параметры["ФИОЧленаКомиссии"] = СТЧ.Пользователи;
			ТабличныйДокумент.Вывести(ОбластьПодвалОписиЧленыКомисии);
		КонецЕсли;
	КонецЦикла;
	Если ЧленКомиссии <=3 Тогда
		ТабличныйДокумент.Вывести(ОбластьПодвалОписи);
	КонецЕсли;
    ОбластьПодвалОписи1.Параметры.НачальныйНомерПоПорядку = 1;
    ОбластьПодвалОписи1.Параметры.КонечныйНомерПоПорядку	 = НомерСтроки;
		
	ОбластьПодвалОписи1.Параметры.ДолжностьМОЛ1           = Шапка.ДолжностьКладовщика;
	ОбластьПодвалОписи1.Параметры.ФИОМОЛ1                 = ФизическиеЛицаУТ.ФамилияИнициалыФизЛица(Шапка.Кладовщик, Шапка.ДатаДокумента);
	
	ОбластьПодвалОписи1.Параметры.ДолжностьРасчетчика	 = НСтр("ru='Главный бухгалтер'");
	ОбластьПодвалОписи1.Параметры.ФИОРасчетчика		     = Шапка.ГлавныйБухгалтер;
	
	
	ТабличныйДокумент.ВывестиГоризонтальныйРазделительСтраниц();
	ТабличныйДокумент.Вывести(ОбластьПодвалОписи1);
